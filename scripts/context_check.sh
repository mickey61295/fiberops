...
check "agent tools (inline+factory+docTool + M9 get_live_activity + M19-C ×33 + M20 attendance ×2 + M21 waste + M23 e-invoice + M26 cancel-irn + M31 working-days + M33 get_bundle + M35 daily-digest + M39 JWL ×2 + M40 PAY ×6)" "238" "$TOOLS"
...
check "docTool delegates (+ M6-C lifecycle ×4 + M6-D ×3 + M20 attendance + M21 waste + M23 e-invoice + M26 cancel-irn + M39 bill_jobwork + M40 PAY ×6)" "62"    "$DOCTOOLS"
...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement + M40 supplier-bill)" "134"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement + M40 /accounts/bill ×2)" "168"    "$LIVEROUTES"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement)" "26"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement)" "36"       "$REGSVCFILES"
check "master configs (24 M2 + shift + 5 ADR-016 + 11 M19-C)" "41"      "$MASTERCFGS"
check "shared zod schema files (+ M6-D dispatch/transfer variants + M20 attendance + M23 e-invoice + M40 supplier-bill)" "42"      "$SCHEMAFILES"
check "posting service files (+ M20 attendance + M39 jobwork-bill + M40 supplier-bill)"      "38"      "$POSTINGSVCS"
check "chain stages"               "15"      "$CHAINSTAGES"
check "doc config files (SPEC-M3 19 + M5-A 5 + M5-B 13 + M5-D 10 + M6-B 1 + M6-D 2; M21 waste rides inventory-variants; M40 supplier-bill)" "41"       "$DOCCFGS"
...
check "commit doors routing through runCommit (M15 ×13 + M23 e-invoice door; M26 cancel rides the same file)" "14" "$(grep -rl 'runCommit' src/app src/lib/erp/doc-actions.ts src/lib/erp/cancel-action.ts 2>/dev/null | grep -v audit.ts | wc -l)"
...
check "m40 PROMPT_VERSION (the money loop rewrite)" "1" "$(grep -c "PROMPT_VERSION = 'm40-2026-09-01'" src/lib/agent/prompt.ts)"
