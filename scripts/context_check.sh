...
check "docTool delegates (+ M6-C lifecycle ×4 + M6-D ×3 + M20 attendance + M21 waste + M23 e-invoice + M26 cancel-irn + M39 bill_jobwork + M40 PAY ×6 + M41 PRC ×3 + M42 INV ×3 + M43 PRG ×2 + M46 L-02 payroll ×2 + M55 L-06 post_shift_wages + M56 PAY-08 post_cheque_clear/bounce)" "75"    "$DOCTOOLS"
check "prisma models (54 + ADR-015 ×7 + ADR-016 ×4 + ADR-019 ×11 + M15 AuditLog + M20 Attendance + M37 IdempotencyKey + M39 JobworkLine + M40 PAY ×3 + M42 StockTake/StockTakeLine + M43 OrderDelivery + M44 CostComponent/CostSheetLine + M46 PayrollRun/PayrollLine + M50 Account + M54 ExpenseHead + M58 ChatSession/ChatMessage + M60 LoginAttempt/LoginAudit)" "96"      "$MODELS"
check "erp view/shell components (+print-button +lifecycle-form +approval-queue +M8-A print trio +M9 live-tracker +M17 register-rows +M18 command-palette +M18-C doc-view-actions/change-password +M14 live-stream-tracker +M22 keypad-mode + M41 dc-lifecycle-actions + M57 profile-name-form/profile-voice-prefs + M60 profile-sessions)" "39" "$VIEWS"
check "archetype engines (+register-screen +report-screen)" "4"       "$ARCHETYPES"
check "pipeline tests"             "15"      "$TESTS"
check "menu registry tests (M5-D + M6-A/B/C/D + M7-C findGroupForPath + M9 + M11 blocks + M59 admin-hub + M60 login-audit)" "31"      "$REGTESTS"
check "master config tests"        "8"       "$CFGTESTS"
check "master parity test blocks"   "8"       "$PARITYTESTS"  # loop-generated at runtime (41 masters) + the M61 E-3.1 taken-code refusal case
...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement + M40 supplier-bill + M41 PRC ×3 + M42 INV ×2 + M43 PRG program-propose + M45 L-01 operator-statement + M46 L-02 payroll + M48 L-03 statutory + M52 M-03 final-accounts quartet + M55 L-06 shift-wages + M56 PAY-08 pdc + M59 admin-hub + M60 login-audit)" "151"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement + M40 /accounts/bill ×2 + M41 PRC ×3 + M42 INV ×3 + M43 /programs/propose + M45 /hr/operator-statement + M46 /hr/payroll ×2 + M48 /hr/statutory + M52 M-03 /accounts/trial-balance + day-book + cash-book + final-accounts + M55 /hr/shift-wages + M56 /accounts/pdc + M57 /profile + M59 /admin + M60 /admin/login-audit)" "188"    "$LIVEROUTES"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement + M41 despatch-register + M42 waste-percent + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet + M55 shift-wages + M56 pdc + M60 login-audit)" "38"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent + M43 program-proposal + M44 cost-compare + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet + M55 shift-wages + M56 pdc + M60 login-audit)" "50"       "$REGSVCFILES"
check "master configs (24 M2 + shift + 5 ADR-016 + 11 M19-C + M44 cost-component + M50 account + M54 expense-head)" "44"      "$MASTERCFGS"
...
check "posting service files (+ M20 attendance + M39 jobwork-bill + M40 supplier-bill + M41 purchase-return + M42 stock-take + M43 order-deliveries + program-spec + M45 employee-party + M46 payroll + M56 cheque lifecycle)"      "45"      "$POSTINGSVCS"
check "chain stages"               "15"      "$CHAINSTAGES"
check "doc config files (SPEC-M3 19 + M5-A 5 + M5-B 13 + M5-D 10 + M6-B 1 + M6-D 2; M21 waste rides inventory-variants; M40 supplier-bill; M41 purchase-return)" "42"       "$DOCCFGS"
check "MAX_STEPS"                  "12"      "$MAXSTEPS"
check "m7-waveA auth lib files (password/session/current-user + api-guard + M60 security)" "5" "$(ls src/lib/auth/*.ts 2>/dev/null | grep -v rights.ts | grep -v login-cookies.ts | wc -l)"
...
check "m7-waveA auth api routes (login/logout/session/bootstrap + M18-C change-password + M57 profile + M60 sessions/clear)" "7" "$(ls -d src/app/api/auth/*/ 2>/dev/null | grep -v admin | wc -l)"
check "m7-waveC admin api routes (set-password + M60 clear-lockout)" "2" "$(ls src/app/api/auth/admin/set-password/route.ts src/app/api/auth/admin/clear-lockout/route.ts 2>/dev/null | wc -l)"
...
check "m8 print doc families in registry (Wave A 5 + Wave B 15 + M18 order + M33 bundle labels ×2 + M42 stock-take count sheet + M46 payslip)" "25" "$(grep -cE "^  '?[a-z-]+'?: fetch" src/lib/erp/print/index.ts)"
...
check "m8 print doors on doc view pages (Wave A 5 + Wave B 14 files + M18 Order Hub; gate-view covers 2 routes; M42 stock-take view + M46 payroll run view)" "22" "$(grep -rl 'DocPrintLink' 'src/app/(erp)' --include='page.tsx' --include='gate-view.tsx' 2>/dev/null | wc -l)"
...
check "m13 flags registry count (28 LLD-07 + 4 notification + M41 po_appr + M42 INV ×5 + M43 multi_style_orders)" "39" "$(grep -c '  { name:' src/lib/erp/flags.ts)"
...
check "m14 createdAt indexes (16 feed families + StockLedger + M15 AuditLog + M20 Attendance + M40 PaymentAllocation + M46 PayrollRun)" "21" "$(grep -c '@@index(\[createdAt\])' prisma/schema.prisma)"
...
check "commit doors routing through runCommit (M15 ×13 + M23 e-invoice door; M26 cancel rides the same file; M41 po-amend + dc-transition doors + the amendments page doc-comment; M42 stock-take actions; M43 propose/delivery/spec doors; M46 payroll actions)" "22" "$(grep -rl 'runCommit' src/app src/lib/erp/doc-actions.ts src/lib/erp/cancel-action.ts 2>/dev/null | grep -v audit.ts | wc -l)"
...
for f in docs/CONTEXT/00-START-HERE.md docs/CONTEXT/01-STATE.md \
         ...
         docs/CONTEXT/specs/SPEC-M19.md scripts/gen_m19_pages.mjs \
         src/lib/erp/register-configs/wave-b.ts \
         src/lib/erp/registers/cutting.ts \
         src/lib/erp/registers/line-issues.ts \
         src/lib/erp/registers/supplier-pending.ts \
         src/lib/erp/registers/po-register.ts \
         src/lib/erp/registers/supplier-history.ts \
         'src/app/(erp)/cutting/register/page.tsx' \
         'src/app/(erp)/production/issue/register/page.tsx' \
         'src/app/(erp)/procurement/supplier-pending/page.tsx' \
         ...
         docs/CONTEXT/specs/SPEC-M14.md \
         src/lib/erp/audit.ts \
         src/lib/erp/registers/audit-log.ts \
         ...
         docs/CONTEXT/specs/SPEC-M16.md \
         src/lib/erp/schemas/attendance.ts \
         src/lib/erp/posting/attendance.ts \
         src/lib/erp/registers/attendance.ts \
         src/lib/erp/register-configs/attendance.ts \
         'src/app/(erp)/hr/attendance/page.tsx' \
         'src/app/(erp)/hr/attendance/csv/route.ts' \
         tests/unit/attendance.test.ts scripts/route_smoke_m20.sh \
