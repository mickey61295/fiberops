...
check "agent tools (inline+factory+docTool + M9 get_live_activity + M19-C ×33 + M20 attendance ×2 + M21 waste + M23 e-invoice + M26 cancel-irn + M31 working-days + M33 get_bundle + M35 daily-digest + M39 JWL ×2)" "232" "$TOOLS"
...
check "docTool delegates (+ M6-C lifecycle ×4 + M6-D ×3 + M20 attendance + M21 waste + M23 e-invoice + M26 cancel-irn + M39 bill_jobwork)" "56"    "$DOCTOOLS"
check "prisma models (54 + ADR-015 ×7 + ADR-016 ×4 + ADR-019 ×11 + M15 AuditLog + M20 Attendance + M37 IdempotencyKey + M39 JobworkLine)" "80"      "$MODELS"
...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement)" "133"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement)" "166"    "$LIVEROUTES"
...
check "posting service files (+ M20 attendance + M39 jobwork-bill)"      "37"      "$POSTINGSVCS"
