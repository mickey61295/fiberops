...
check "agent tools (inline+factory+docTool + M9 get_live_activity + M19-C ×33 + M20 attendance ×2 + M21 waste + M23 e-invoice + M26 cancel-irn + M31 working-days + M33 get_bundle + M35 daily-digest + M39 JWL ×2 + M40 PAY ×6 + M41 PRC ×5 + M42 INV ×3 + M43 PRG ×3 + M44 CST ×4 + M45 L-01 operator-statement + M46 L-02 payroll trio)" "257" "$TOOLS"
...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement + M40 supplier-bill + M41 PRC ×3 + M42 INV ×2 + M43 PRG program-propose + M45 L-01 operator-statement + M46 L-02 payroll)" "142"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement + M40 /accounts/bill ×2 + M41 PRC ×3 + M42 INV ×3 + M43 /programs/propose + M45 /hr/operator-statement + M46 /hr/payroll ×2)" "178"    "$LIVEROUTES"
...
check "m47 PROMPT_VERSION (the side_quest merge: m44 costing lines + m46 payroll lines)" "1" "$(grep -c "PROMPT_VERSION = 'm47-2026-09-06'" src/lib/agent/prompt.ts)"
