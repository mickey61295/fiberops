...
check "erp view/shell components (+print-button +lifecycle-form +approval-queue +M8-A print trio +M9 live-tracker +P0 register-rows)" "30"      "$VIEWS"
...
check "p0 SLUG_PRINT_DOC entries (verified slug→printDocType pairs; 'order' deliberately absent = M13 gap)" "20" "$(sed -n '/^export const SLUG_PRINT_DOC/,/^}/p' src/components/archetypes/doc-screen.tsx | grep -cE "^  '?[a-z-]+'?: '[a-z-]+'," )"
check "p0 keyboard reflex unit tests" "20" "$(grep -c '^  it(' tests/unit/keyboard-reflex.test.ts)"
check "p0 doc-screen F-key handlers (F2 save / F4 field / F9 print / Esc edit)" "4" "$(grep -cE "e.key === '(F2|F4|F9|Escape)'" src/components/archetypes/doc-screen.tsx)"
check "p0 Enter-swallow + row-spawn in doc-screen form" "2" "$(grep -cE "(e.key !== 'Enter' \|\| e.defaultPrevented|pendingFocus.current = \{ r: ri \+ 1)" src/components/archetypes/doc-screen.tsx)"
check "p0 register keyboard rows component" "5" "$(grep -cE "e.key === '(ArrowDown|ArrowUp|Enter|Home|End)'" src/components/erp/register-rows.tsx)"
check "p0 master-table row cursor" "5" "$(grep -cE "e.key === '(ArrowDown|ArrowUp|Enter|Home|End)'" src/components/archetypes/master-table.tsx)"
check "p0 global / listener + data-slash opts (app-shell listener + master search + filter bar from/q)" "6" "$(grep -c "data-slash" src/components/erp/app-shell.tsx src/components/archetypes/master-table.tsx src/components/erp/register-filter-bar.tsx | awk -F: '{s+=$2} END {print s+1}')"
check "p0 Despatch spelling (group label + 7 breadcrumbs)" "8" "$(grep -rc "Despatch" src/lib/erp/menu-registry.ts $(ls 'src/app/(erp)/dispatch'/*/page.tsx 'src/app/(erp)/dispatch'/*/*/page.tsx 2>/dev/null) 2>/dev/null | awk -F: '{s+=($2>0)?1:0} END {print s}')"
