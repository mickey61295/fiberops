...
check "prisma models (54 + ADR-015 ×7 + ADR-016 ×4 + ADR-019 ×11 + M15 AuditLog + M20 Attendance + M37 IdempotencyKey + M39 JobworkLine + M40 PAY ×3 + M42 StockTake/StockTakeLine + M43 OrderDelivery + M44 CostComponent/CostSheetLine + M46 PayrollRun/PayrollLine + M50 Account + M54 ExpenseHead + M58 ChatSession/ChatMessage)" "94"      "$MODELS"
check "erp view/shell components (+print-button +lifecycle-form +approval-queue +M8-A print trio +M9 live-tracker +M17 register-rows +M18 command-palette +M18-C doc-view-actions/change-password +M14 live-stream-tracker +M22 keypad-mode + M41 dc-lifecycle-actions + M57 profile-name-form/profile-voice-prefs)" "38" "$VIEWS"
check "archetype engines (+register-screen +report-screen)" "4"       "$ARCHETYPES"
check "pipeline tests"             "15"      "$TESTS"
check "menu registry tests (M5-D + M6-A/B/C/D + M7-C findGroupForPath + M9 + M11 blocks + M59 admin-hub)" "30"      "$REGTESTS"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement + M41 despatch-register + M42 waste-percent + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet + M55 shift-wages + M56 pdc)" "37"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent + M43 program-proposal + M44 cost-compare + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet + M55 shift-wages + M56 pdc)" "49"       "$REGSVCFILES"
...
check "m7-waveA auth api routes (login/logout/session/bootstrap + M18-C change-password + M57 profile)" "6" "$(ls -d src/app/api/auth/*/ 2>/dev/null | grep -v admin | wc -l)"
