...
check "agent tools (inline+factory+docTool + M9 get_live_activity + M19-C ×33 + M20 attendance ×2 + M21 waste + M23 e-invoice + M26 cancel-irn + M31 working-days + M33 get_bundle + M35 daily-digest + M39 JWL ×2 + M40 PAY ×6 + M41 PRC ×5 + M42 INV ×3)" "246" "$TOOLS"
...
check "docTool delegates (+ M6-C lifecycle ×4 + M6-D ×3 + M20 attendance + M21 waste + M23 e-invoice + M26 cancel-irn + M39 bill_jobwork + M40 PAY ×6 + M41 PRC ×3 + M42 INV ×3)" "68"    "$DOCTOOLS"
check "prisma models (54 + ADR-015 ×7 + ADR-016 ×4 + ADR-019 ×11 + M15 AuditLog + M20 Attendance + M37 IdempotencyKey + M39 JobworkLine + M40 PAY ×3 + M42 StockTake/StockTakeLine)" "85"      "$MODELS"
...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement + M40 supplier-bill + M41 PRC ×3 + M42 INV ×2)" "139"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement + M40 /accounts/bill ×2 + M41 PRC ×3 + M42 INV ×3)" "174"    "$LIVEROUTES"
...
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent)" "38"       "$REGSVCFILES"
...
check "posting service files (+ M20 attendance + M39 jobwork-bill + M40 supplier-bill + M41 purchase-return + M42 stock-take)"      "40"      "$POSTINGSVCS"
...
check "m11+m13 flag registry defs (28 LLD-07 + 4 notification + M41 po_appr + M42 INV ×5)" "38" "$(grep -c "name: '" src/lib/erp/flags.ts)"
...
check "m13 flags registry count (28 LLD-07 + 4 notification + M41 po_appr + M42 INV ×5)" "38" "$(grep -c '  { name:' src/lib/erp/flags.ts)"
