...
check "docTool delegates (+ M6-C lifecycle ×4 + M6-D ×3 + M20 attendance + M21 waste + M23 e-invoice + M26 cancel-irn + M39 bill_jobwork + M40 PAY ×6 + M41 PRC ×3 + M42 INV ×3 + M43 PRG ×2 + M46 L-02 payroll ×2 + M55 L-06 post_shift_wages)" "73"    "$DOCTOOLS"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement + M41 despatch-register + M42 waste-percent + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet + M55 shift-wages)" "36"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent + M43 program-proposal + M44 cost-compare + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet + M55 shift-wages)" "48"       "$REGSVCFILES"
