...
check "menu registry tests"        "17"      "$REGTESTS"
...
check "live routes (M4 Wave B)"    "64"      "$LIVEROUTES"
check "register config files (M4 Wave B fleet)" "16"       "$REGCFGS"
check "register service files (M4 Wave B fleet + order-status)" "17"       "$REGSVCFILES"
