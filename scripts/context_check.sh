...
check "agent tools (inline+factory+docTool + M9 get_live_activity + M19-C ×33 + M20 attendance ×2 + M21 waste + M23 e-invoice + M26 cancel-irn + M31 working-days + M33 get_bundle + M35 daily-digest + M39 JWL ×2 + M40 PAY ×6 + M41 PRC ×5 + M42 INV ×3 + M43 PRG ×3 + M45 L-01 operator-statement)" "250" "$TOOLS"
...
check "prisma models (54 + ADR-015 ×7 + ADR-016 ×4 + ADR-019 ×11 + M15 AuditLog + M20 Attendance + M37 IdempotencyKey + M39 JobworkLine + M40 PAY ×3 + M42 StockTake/StockTakeLine + M43 OrderDelivery)" "86"      "$MODELS"
...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement + M40 supplier-bill + M41 PRC ×3 + M42 INV ×2 + M43 PRG program-propose + M45 L-01 operator-statement)" "141"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement + M40 /accounts/bill ×2 + M41 PRC ×3 + M42 INV ×3 + M43 /programs/propose + M45 /hr/operator-statement)" "176"    "$LIVEROUTES"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement + M41 despatch-register + M42 waste-percent + M45 operator-statement)" "29"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent + M43 program-proposal + M45 operator-statement)" "40"       "$REGSVCFILES"
...
check "posting service files (+ M20 attendance + M39 jobwork-bill + M40 supplier-bill + M41 purchase-return + M42 stock-take + M43 order-deliveries + program-spec + M45 employee-party)"      "43"      "$POSTINGSVCS"
...
check "m8 print doc families in registry (Wave A 5 + Wave B 15 + M18 order + M33 bundle labels ×2 + M42 stock-take count sheet)" "24" "$(grep -cE "^  '?[a-z-]+'?: fetch" src/lib/erp/print/index.ts)"
...
check "m8 print doors on doc view pages (Wave A 5 + Wave B 14 files + M18 Order Hub; gate-view covers 2 routes; M42 stock-take view)" "21" "$(grep -rl 'DocPrintLink' 'src/app/(erp)' --include='page.tsx' --include='gate-view.tsx' 2>/dev/null | wc -l)"
...
check "m14 createdAt indexes (16 feed families + StockLedger + M15 AuditLog + M20 Attendance + M40 PaymentAllocation)" "20" "$(grep -c '@@index(\[createdAt\])' prisma/schema.prisma)"
...
check "commit doors routing through runCommit (M15 ×13 + M23 e-invoice door; M26 cancel rides the same file; M41 po-amend + dc-transition doors + the amendments page doc-comment; M42 stock-take actions; M43 propose/delivery/spec doors)" "21" "$(grep -rl 'runCommit' src/app src/lib/erp/doc-actions.ts src/lib/erp/cancel-action.ts 2>/dev/null | grep -v audit.ts | wc -l)"
...
check "m45 PROMPT_VERSION (wage reconciliation — the operator statement)" "1" "$(grep -c "PROMPT_VERSION = 'm45-2026-09-02'" src/lib/agent/prompt.ts)"
