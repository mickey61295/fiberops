...
check "prisma models (54 + ADR-015 ×7 + ADR-016 ×4 + ADR-019 ×11 + M15 AuditLog + M20 Attendance + M37 IdempotencyKey + M39 JobworkLine + M40 PAY ×3 + M42 StockTake/StockTakeLine + M43 OrderDelivery + M44 CostComponent/CostSheetLine + M46 PayrollRun/PayrollLine + M50 Account)" "91"      "$MODELS"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement + M41 despatch-register + M42 waste-percent + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet)" "35"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent + M43 program-proposal + M44 cost-compare + M45 operator-statement + M46 payroll + M48 statutory + M52 M-03 final-accounts quartet)" "47"       "$REGSVCFILES"
check "master configs (24 M2 + shift + 5 ADR-016 + 11 M19-C + M44 cost-component + M50 account)" "43"      "$MASTERCFGS"
...
check "m53 PROMPT_VERSION (M-04 Tally both sides: the export doctrine — counted once, cancels ride their CN- reversals, GST split ledgers, get_tally_export)" "1" "$(grep -c "PROMPT_VERSION = 'm53-2026-09-06'" src/lib/agent/prompt.ts)"
