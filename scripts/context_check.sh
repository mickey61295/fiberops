...
check "erp view/shell components (+print-button +lifecycle-form +approval-queue +M8-A print trio +M9 live-tracker +M17 register-rows +M18 command-palette +M18-C doc-view-actions/change-password +M14 live-stream-tracker +M22 keypad-mode + M41 dc-lifecycle-actions)" "36" "$VIEWS"
...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement + M40 supplier-bill + M41 PRC ×3 + M42 INV ×2 + M43 PRG program-propose + M45 L-01 operator-statement + M46 L-02 payroll + M47 L-03 statutory ×2)" "144"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement + M40 /accounts/bill ×2 + M41 PRC ×3 + M42 INV ×3 + M43 /programs/propose + M45 /hr/operator-statement + M46 /hr/payroll ×2 + M47 /hr/statutory + /admin/statutory)" "180"    "$LIVEROUTES"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement + M41 despatch-register + M42 waste-percent + M45 operator-statement + M46 payroll + M47 statutory)" "31"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent + M43 program-proposal + M45 operator-statement + M46 payroll + M47 statutory)" "42"       "$REGSVCFILES"
...
check "m47 PROMPT_VERSION (statutory payroll — Module L Batch 3)" "1" "$(grep -c "PROMPT_VERSION = 'm47-2026-09-08'" src/lib/agent/prompt.ts)"
check "m47 statutory config module (stat: registry — getStatutoryPure pure read)" "1" "$(ls src/lib/erp/statutory.ts 2>/dev/null | wc -l)"
check "m47 statutory rate defs (4 heads × rates/thresholds)" "17" "$(grep -c "valueType: 'number'\|valueType: 'boolean'" src/lib/erp/statutory.ts)"
check "m47 statutory admin board (page + client + action)" "3" "$(ls 'src/app/(erp)/admin/statutory/page.tsx' 'src/app/(erp)/admin/statutory/statutory-admin.tsx' 'src/app/(erp)/admin/statutory/actions.ts' 2>/dev/null | wc -l)"
check "m47 statutory register screen + challan csv route" "2" "$(ls 'src/app/(erp)/hr/statutory/page.tsx' 'src/app/(erp)/hr/statutory/csv/route.ts' 2>/dev/null | wc -l)"
check "m47 PayrollLine statutory columns (13 frozen legs)" "13" "$(grep -cE '^[[:space:]]*(pfWages|pfEe|pfEr|pfEps|pfEpf|pfEdli|pfAdmin|esiEe|esiEr|ptAmt|lwfEe|lwfEr|statDeduction)[[:space:]]+Float' prisma/schema.prisma)"
check "m47 Journal.partySide column (side-aware ledger)" "1" "$(grep -c 'partySide     String?' prisma/schema.prisma)"
check "m47 payroll-l03 test file (25 tests)" "25" "$(grep -c '^  it(' tests/pipeline/payroll-l03.test.ts 2>/dev/null)"
