...
check "menu items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest + M15 audit + M20 attendance + M21 waste + M39 jobworker-statement + M40 supplier-bill + M41 PRC ×3 + M42 INV ×2 + M43 PRG program-propose + M45 L-01 operator-statement + M46 L-02 payroll + M48 L-03 statutory)" "143"     "$MENUITEMS"
check "live routes (M6 113/113 + M9 /tracker + M11 /admin/settings + M19 ×13 + tally + M13 digest + M14 /live + M15 audit + M20 /hr/attendance + M21 /inventory/waste-receipt + M39 /jobwork/statement + M40 /accounts/bill ×2 + M41 PRC ×3 + M42 INV ×3 + M43 /programs/propose + M45 /hr/operator-statement + M46 /hr/payroll ×2 + M48 /hr/statutory)" "179"    "$LIVEROUTES"
...
check "register config files (M4 + M5 + M6-C + M19 material-stock + wave-b + closing-stock + audit-log + M20 attendance + M39 jobworker-statement + M41 despatch-register + M42 waste-percent + M45 operator-statement + M46 payroll + M48 statutory)" "31"       "$REGCFGS"
check "register service files (M4 + order-status + recon + M5 + M6-C ×2 + M19 ×10 + audit + M20 attendance + M39 jobworker-statement + M41 despatch + M42 waste-percent + M43 program-proposal + M44 cost-compare + M45 operator-statement + M46 payroll + M48 statutory)" "43"       "$REGSVCFILES"
...
check "m50 PROMPT_VERSION (M-01 chart of accounts: journal legs resolve by name/code, unknown refused)" "1" "$(grep -c "PROMPT_VERSION = 'm50-2026-09-06'" src/lib/agent/prompt.ts)"
