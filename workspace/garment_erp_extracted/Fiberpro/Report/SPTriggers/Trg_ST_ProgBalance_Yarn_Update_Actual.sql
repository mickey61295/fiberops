...
; Create By  :  ASLAM            
; Description  :  Trigger for Actual Entry UpdateFlg in ST_ProgBalance_Yarn  
; Change Person  :  ASLAM          
; Last Change Date :  04/Nov/2019 10.00 AM            
; =============================================   */     
CREATE TRIGGER Trg_ST_ProgBalance_Yarn_Update_Actual   ON  ST_ProgBalance_Yarn AFTER UPDATE AS 
BEGIN
    SET NOCOUNT ON;
    ...
    BEGIN
		...
		begin
			Update ST_ProgBalance_Yarn SET ActualPosting_UpdateFlg = 1 Where OrdID=@Ordid And DeptID = @DeptId And CountID = @CountID And  ColID = @ColId
		end
