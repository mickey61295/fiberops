...
; Last Change Date :  10/03/2025 10.30 AM 
; =============================================  */  

CREATE VIEW VUE_DEL_PRSRT AS 
...
SELECT    Trs_Del1.ID, CASE WHEN len(rtrim(dcpre)) < 1 or isnull(len(rtrim(dcpre)),'')=''  THEN RTRIM(CAST(Trs_Del1.DocNo AS varchar)) + '/' + Trs_Del1.Finyear ELSE rtrim(dcpre) 
                      ...
                      Mas_Color.ColorDesc, Mas_Count.CountName, Trs_Del1.remark, Trs_Del1.ProcessType, Trs_Del1.LotNo, StockTable.LotNo AS StkLotno, 
