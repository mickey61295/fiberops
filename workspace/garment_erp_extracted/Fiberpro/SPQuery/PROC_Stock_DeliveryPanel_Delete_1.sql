...
CREATE PROCEDURE PROC_Stock_DeliveryPanel_Delete_1 (@ID Int,@StyleNo Varchar(20),@PartId int,@ColId Int,@SizeId Int,@SourceStageID Int,@Pcs Int,@LotNo Varchar(15),@compId int) AS  DECLARE @Coycode Int,@Ordid Int,@StageId Int,@GodId Int,@StockQty Int,@FinalStage Char (1),@SeqNo int,@PartyId Int,@PcsStockId Int,@ProcessType Char(1),@RejectionTypeId Int ,@DelType Varchar(30),@FinishedStageID Int,@LotId int    ,@Prod_Without_Lot_Despatch_WithLot as char(1), @LotwiseStockReqd   char(1)   
...
SELECT Top 1 @FinishedStageID = -3  /* SELECT Top 1 @FinishedStageID = StageId  From Panel_StockTable A INNER JOIN Mas_JobWrkComp B ON A.S
...
Select Id,StyleNo,Colid,PartId,SizeId,IsNull(lotNo,'') LotNo,Pcs,SourceStageId,CompId FROM Trs_Pcs2 Where ID=@Id And StyleNo=@StyleNo and Colid = @ColId and PartId = @PartId And SizeId =@SizeId and SourceStageId = @SourceStageID and LotNo    =@LotNo  and IsNull(CompId,0) = @compId
...
UPDATE Panel_StockTableQty SET StockQty=Panel_StockTableQty.StockQty+@Pcs From Panel_StockTableQty Inner Join Panel_StockTable On Panel_StockTable.PcsStockId=Panel_StockTableQty.PcsStockId Inner Join Trs_Pcs1 On Panel_StockTable.Coycode=Trs_Pcs1.Coycode And Panel_StockTable.OrdId=Trs_Pcs1.Ordjobno And Panel_StockTable.StageId=@FinishedStageID And Panel_StockTable.GodId=Trs_Pcs1.GodId And Panel_StockTable.StyleNo=@StyleNo And Panel_StockTable.PartId=@PartId And Panel_StockTableQty.ColId=@ColId And Panel_StockTableQty.SizeId=@SizeId And Panel_StockTableQty.CompId=@CompId and IsNull(GoodPcsFlag,'G')='G' and IsNull(Panel_StockTableQty.RejectionTypeId,0)=0 And Panel_StockTable.LotId = @LotId WHERE Panel_StockTable.coycode=Trs_Pcs1.Coycode And Panel_StockTable.
Ordid=Trs_Pcs1.Ordjobno and Panel_StockTable.StyleNo=@StyleNo And Panel_StockTable.LotId = @LotId and Panel_StockTable.Stageid=@FinishedStageID And Panel_StockTable.PartId=@PartId and Panel_StockTable.GodId=Trs_Pcs1.GodId and Panel_StockTableQty.Colid=@ColId and Panel_StockTableQty.SizeId=@SizeId And Panel_StockTableQty.CompId=@CompId and IsNull(GoodPcsFlag,'G')='G' and IsNull(Panel_StockTableQty.RejectionTypeId,0)=0 and Partyid=0 And Trs_Pcs1.Id=@Id     
...
UPDATE Panel_StockTableQty SET StockQty=Panel_StockTableQty.StockQty-@Pcs From Panel_StockTableQty Inner Join Panel_StockTable On Panel_StockTable.PcsStockId=Panel_StockTableQty.PcsStockId Inner Join Trs_Pcs1 On Panel_StockTable.Coycode=Trs_Pcs1.Coycode And Panel_StockTable.OrdId=Trs_Pcs1.Ordjobno And Panel_StockTable.StageId=Trs_Pcs1.TargetStageId And Panel_StockTable.GodId=Trs_Pcs1.GodId And Panel_StockTable.StyleNo=@StyleNo And Panel_StockTable.LotId = @LotId And Panel_StockTable.PartId=@PartId And Panel_StockTableQty.ColId=@ColId And Panel_StockTableQty.SizeId=@SizeId And Panel_StockTableQty.CompId=@CompId and IsNull(GoodPcsFlag,'G')=Case When @ProcessType='P' Then 'G' Else 'M' End and IsNull(Panel_StockTableQty.RejectionTypeId,0)=Case When @ProcessType='P' Then 0 Else @RejectionTypeId End WHERE Panel_StockTable.coycode=Trs_Pcs1.Coycode And Panel_StockTable.Ordid=Trs_Pcs1.Ordjobno and Panel_StockTable.StyleNo=@StyleNo And Panel_StockTable.LotId = @LotId and Panel_StockTable.Stageid=Trs_Pcs1.TargetStageid And Panel_StockTable.PartId=@PartId and Panel_StockTable.GodId=Trs_Pcs1.GodId and Panel_StockTableQty.Colid=@ColId and Panel_StockTableQty.SizeId=@SizeId And Panel_StockTableQty.CompId=@CompId and IsNull(GoodPcsFlag,'G')=Case When @ProcessType='P' Then 'G' Else 'M' End and IsNull(Panel_StockTableQty.RejectionTypeId,0)=Case When @ProcessType='P' Then 0 Else @RejectionTypeId End and Partyid=@PartyId And Trs_Pcs1.Id=@Id   And Panel_StockTable.LotId = @LotId  

End   

If @SourceStageid<>0 And (Select IsNull(PcsType,'Piece') From Mas_JobWrkComp Where Id=@StageId)='Piece' OR (Select Rtrim(IsNull(PcsType,'Piece')) From Mas_JobWrkComp Where Id=@StageId)='Bit' Or (@Stageid=@SourceStageid) OR   
...
Begin /*Supplier Receipt



 Rejection */ 	/*Insert into tmp_trg Values ('START21')*/ 	

Select @PcsStockId=PcsStockId From Panel_StockTable where coycode=@coycode and Ordid=@Ordid and StyleNo=@StyleNo And LotID = @Lotid and Stageid=@Stageid and PartId=@PartId and GodId=@GodId and PartyId=0  	

If EXISTS (select * from Panel_StockTable Inner Join Panel_StockTableQty On Panel_StockTable.PcsStockId=Panel_StockTableQty.PcsStockId where coycode=@coycode and Ordid=@Ordid and StyleNo=@StyleNo And LotID = @LotID and Stageid=@SourceStageid and 

PartId=@PartId and GodId=@GodId and PartyId=0 and Panel_StockTableQty.ColId=@Colid and Panel_StockTableQty.SizeId=@SizeId And Panel_StockTableQty.CompId=@CompId and IsNull(GoodPcsFlag,'G')= 'G' and IsNull(RejectionTypeId,0)=0)  

Begin 

UPDATE Panel_StockTableQty SET StockQty=Panel_StockTableQty.StockQty+@Pcs From



 Panel_StockTableQty Inner Join Panel_StockTable On Panel_StockTable.PcsStockId=Panel_StockTableQty.PcsStockId Inner Join Trs_Pcs1 On Panel_StockTable.Coycode=Trs_Pcs1.Coycode And Panel_StockTable.OrdId=Trs_Pcs1.Ordjobno And Panel_StockTable.GodId=Trs_Pcs1.GodId And Panel_StockTable.StyleNo=@StyleNo And Panel_StockTable.LotId = @LotId And Panel_StockTable.PartId=@PartId And Panel_StockTableQty.ColId=@ColId And Panel_StockTableQty.SizeId=@SizeId And Panel_StockTableQty.CompId=@CompId and IsNull(GoodPcsFlag,'G')='G' and IsNull(Panel_StockTableQty.RejectionTypeId,0)=0 WHERE Panel_StockTable.coycode=Trs_Pcs1.Coycode And Panel_StockTable.Ordid=Trs_Pcs1.Ordjobno and Panel_StockTable.StyleNo=@StyleNo And Panel_StockTable.LotId = @LotId And Panel_StockTable.PartId=@PartId and Panel_StockTable.GodId=Trs_Pcs1.GodId and Panel_StockTableQty.Colid=@ColId and Panel_StockTableQty.SizeId=@SizeId And Panel_StockTableQty.CompId=@CompId  and IsNull(GoodPcsFlag,'G')='G' and IsNull(Panel_StockTableQty.RejectionTypeId,0)=0 and


 Partyid=0 And Panel_StockTable.StageId=@SourceStageId And Trs_Pcs1.Id=@Id  
...
INSERT INTO Panel_StockTable (Coycode,Ordid,styleNo,Stageid,PartId,SeqNo,GodId,PcsStockId,PartyId,LotID) VALUES (@Coycode,@Ordid,@StyleNo,@Stageid,@PartId,@SeqNo,@GodId,@PcsStockId,0,@LotID) INSERT INTO Panel_StockTableQty (PcsStockId,colid,Sizeid,StockQty,GoodPcsFlag,RejectionTypeId,CompId) VALUES (@PcsStockId,@ColId,@Sizeid,@StockQty,Case When @ProcessType='P' Then 'G' Else '



M' End,Case When @ProcessType='P' Then 0 Else @RejectionTypeId End,@CompId) End  End   FETCH NEXT FROM LINE_CURSOR INTO @Id,@StyleNo,@Colid,@PartId,@SizeId,@LotNo,@Pcs,@SourceStageId,@CompId   END   CLOSE LINE_CURSOR    DEALLOCATE LINE_CURSOR   SET NOCOUNT OFF  END 
