...
D.StyleNo And A.Partid = D.PartId and C.JobOrdid = D.JobId AND Trs_AddPanelEntryQty_Component.CompId = D.CompId WHERE A.Id = @Id --and D.CompId = @CompId



ELSE

INSERT INTO #DTORDERDTL  Select A.Id,SizID,ProdPcs,d.CompId FROM Trs_AddPanelEntry A INNER JOIN Trs_AddPanelEntryQty B ON a.ID = B.ID INNER JOIN Trs_AddPanelEntryQty_Component C ON A.Id = C.ID INNER JOIN Prod_CutComponents D ON A.Ordid = D.OrdId And A.StyleNo = D.StyleNo And A.Partid = D.PartId and C.CompId = D.CompId  WHERE A.Id = @Id And D.CompId = @CompId
