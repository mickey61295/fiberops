...
; Change Person    :  ASLAM 
; Last Change Date :  12-JAN-2026 09.28 AM
; ============================================= */    

CREATE PROCEDURE Proc_Rpt_OCR_Summary(@Guid varchar(256),@Ordid int)    
...
Insert into Tmp_OCRSummary (guid,Ordid,Deptid,ReqKgs,DeptSlno) select @guid,knitt.Ordid,deptid,Sum(ReqKgs) as ReqKgs ,seq.sl  from Pro_Reqknitt knitt (nolock) left outer join OrdSeq seq(nolock) on knitt.ordid=seq.ordid and knitt.DeptId=seq.Prs where knitt.ordid=@Ordid group by  knitt.Ordid,deptid, seq.sl order by seq.sl      
...
update Tmp_OCRSummary set DelKgs = x.DelKgs from   (select ordid,Prs_dept,sum(kg) as DelKgs from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id   where trtype=1 And ProcessType='P' and Ordid= @Ordid group by Ordid,Prs_dept )x   inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and tmp.Guid=@guid     


/* YARN PO Details as Delivery Side */
update Tmp_OCRSummary set DelKgs = isNull(tmp.Delkgs,0)+ x.DelKgs from   (select ordid,Dept,sum(dtl.PoQty) as DelKgs from Trs_Po1 MAs(nolock)  inner join Trs_po2 dtl (nolock) on Mas.id=Dtl.id   where Ordid= @Ordid group by Ordid,dept )x   inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.dept=tmp.deptid and tmp.Guid=@guid     


/* FABRIC PO Details as Delivery Side */
update Tmp_OCRSummary set DelKgs = isNull(tmp.Delkgs,0)+ x.DelKgs from   (select ordid,Dept,sum(dtl.PoKgs) as DelKgs from Trs_Po1 MAs(nolock)  inner join Trs_po3 dtl (nolock) on Mas.id=Dtl.id   where Ordid= @Ordid group by Ordid,dept )x   inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.dept=tmp.deptid and tmp.Guid=@guid    

/* Multiprocess Receipt - DC UPdate */


update Tmp_OCRSummary set DelKgs = isNull(tmp.Delkgs,0)+x.DelKgs from   (select ordid,Dtl1.DeptId,sum(Dtl1.DcKgs) as DelKgs from Trs_MultiPrs_Grn1 MAs(nolock)  inner join Trs_MultiPrs_Grn2 dtl (nolock) on Mas.id=Dtl.id  INNER JOIN Trs_MultiPrs_Grn3 Dtl1 ON  Mas.Id =Dtl1.ID and dtl.RowSlno = dtl1.Slno where ProcessType='P'  and Ordid= @Ordid And RowSlno<>1 group by Ordid,Dtl1.deptID)x     inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.DeptID=tmp.deptid and tmp.Guid=@guid  



update Tmp_OCRSummary set RDelKgs = x.DelKgs from (select ordid,Prs_dept,sum(kg) as DelKgs from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id   where trtype=1 And ProcessType='R' and Ordid= @Ordid group by Ordid,Prs_dept )x     inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and tmp.Guid=@guid     
...
update tmp set RecKgs = x.RecKgs from    (select ordid,Dept,sum(RecKgs) as RecKgs from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id   where grntype in ('Process' ,'Purchase') And (ISNULL(processtype,'P')='P' or processtype ='' )and Ordid= @Ordid group by Ordid,Dept )x inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid   and x.Dept=tmp.deptid and tmp.Guid=@guid  
...
update tmp set RecKgs = IsNull(tmp.RecKgs,0)+x.RecKgs from    (select ordid,Dtl1.DeptID as Dept,sum(RecKgs) as RecKgs from Trs_MultiPrs_Grn1 MAs(nolock)  inner join Trs_MultiPrs_Grn2 dtl (nolock) on Mas.id=Dtl.id  INNER JOIN Trs_MultiPrs_Grn3 Dtl1 ON  Mas
...
update tmp set RRecKgs = x.RecKgs from     (select ordid,Dept,sum(RecKgs) as RecKgs from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id   where grntype in ('Process' ,'Purchase') And processtype='R' and Ordid= @Ordid group by Ordid,Dept )x inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid   and x.Dept=tmp.deptid and tmp.Guid=@guid  
...
.Guid=@Guid */
    ...
    update Tmp_OCRSummary set OpeningKgs = IsNull(x.OpeningKgs,0) from  (Select Ordid,Dept,sum(Kgs) as OpeningKgs  from Trs_Opening where Ordid= @Ordid  group by  Ordid,Dept )x  inner  join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Dept=tmp.deptid and
 tmp.Guid=@Guid
 ...
 /* Stock Adjustment */
	...
	update Tmp_OCRSummary set AddStkAdjKgs= IsNull(x.AddStkAdjKgs,0) from   (select ordid,Prs_dept,sum(kg) as AddStkAdjKgs from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id   where trtype=5 And ProcessType='' And dtl.StockAddLess = 'Add' and Ordid= @Ordid group by Ordid,Prs_dept )x   inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and tmp.Guid=@guid    



   	update Tmp_OCRSummary set LessStkAdjKgs= IsNull(x.LessStkAdjKgs,0) from   (select ordid,Prs_dept,sum(kg) as LessStkAdjKgs from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id   where trtype=5 And ProcessType='' And dtl.StockAddLess = 'Less' and Ordid= @Ordid group by Ordid,Prs_dept )x   inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and tmp.Guid=@guid    



	/*Chandru*/
	/*sales*/
		update Tmp_OCRSummary set sales = x.DelKgs from (select dtl.ordid,Dept as prs_dept,isnull(sum(kg),0) as DelKgs from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id   inner join stocktable on stocktable.StockID= dtl.StockID  where  trtype=2 And ProcessType='P' and SalCumInv ='Y' and dtl.Ordid= @Ordid group by dtl.Ordid,Prs_dept,Dept Union select dtl.ordid,Dept as prs_dept,isnull(sum(kg),0) as DelKgs from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id   inner join stocktable on stocktable.StockID= dtl.StockID  where  trtype=2 and  Isnull(SalCumInv,'N') ='N'   and dtl.Ordid= @Ordid group by dtl.Ordid,Prs_dept,Dept )x     inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and tmp.Guid=@guid
  
  /*sales retrn*/
update tmp set sales = sales - isnull(x.RecKgs,0) from     (select dtl.ordid,a.Dept,sum(RecKgs) as RecKgs from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable a on a.StockID =dtl.StockID   where grntype in ('Sales Return')  and dtl.Ordid= @Ordid group by dtl.Ordid,a.Dept )x inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid   and x.Dept=tmp.deptid and tmp.Guid=@guid  
...
update tmp set Retkgs = x.RecKgs from     (select ordid,Dept,sum(RecKgs) as RecKgs from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id   where grntype in ('Process Return') and Ordid= @Ordid group by Ordid,Dept )x inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid  and x.Dept=tmp.deptid and tmp.Guid=@guid     
...
update tmp set Retkgs = (x.AKg + isnull(tmp.Retkgs,0)) from (select OrdId,Prs_Dept,sum(AKg) as AKg from Trs_Del1 Mas (nolock)  INNER JOIN Trs_Del2 Dtl (nolock) ON Mas.ID = Dtl.ID where DELTYPE='R' and OrdId=@Ordid group by OrdId,Prs_Dept) x inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid  and x.Prs_Dept=tmp.deptid and tmp.Guid=@guid
...
update tmp set TRanInKgs = x.TRanInKg from(select TranOrdID as ORdid ,PRs_dePt,sum(Kg) as TRanInKg from trs_del1 MAs(nolock) inner join  trS_del2 dtl(nolock) on MAs.id=Dtl.id where trtype=3 and  TranOrdID= @Ordid group by TranOrdID  ,PRs_dePt ) x     inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and tmp.Guid=@guid     
...
update tmp set TRanOutKgs = x.TRanOutKg from (select OrdID as ORdid ,PRs_dePt,sum(Kg) as TRanOutKg from trs_del1 MAs(nolock) inner join  trS_del2 dtl(nolock) on MAs.id=Dtl.id where trtype=3 and OrdID= @Ordid group by OrdID  ,PRs_dePt ) x     inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and tmp.Guid=@guid     
...
Update Tmp Set ShortageKgs=X.ShortKgs From(Select OrdID,Dept,Sum(ShortKgs) as ShortKgs From Trs_Shortage Where Dept Not In (0,16,17) Group by OrdID,Dept)X INNER JOIN Tmp_OCRSummary Tmp ON  X.Ordid=Tmp.Ordid and X.Dept=Tmp.Deptid and Tmp.Guid=@guid  
...
select @guid,Y.OrdId,stageid,0,0 as ProdPcs,Seq.sl as Sl,PartName,StyleNo from(select ordid,Mas_Dept.DeptID as id,stageid,sum(ProdPcs) as ProdPcs ,mas_dept.OrderSno,PartName,StyleNo from Trs_Prodentry MAs inner join Trs_ProdentryQty Dtl on MAS.id=Dtl.id left outer join Mas_JobWrkComp M_stage on MAS.StageID =M_stage.Id LEFT OUTER JOIN Mas_Dept ON M_stage.DeptId = Mas_Dept.DeptID LEFT OUTER JOIN Mas_Part ON Mas.PartId = Mas_Part.PartId where ordid=@ORdid And ISNULL(Mas_Dept.SEMIFINISH,'S') <> 'F' group by Mas_Dept.Deptid,stageid,ordid,Mas_Dept.OrderSno,PartName,StyleNo) Y left outer join OrdSeq seq(nolock) on Y.ordid=seq.ordid  and Y.id=seq.Prs where id not in (select deptid from Tmp_OCRSummary_Pcs where guid=@guid) group by stageid,Y.ordid,Seq.sl,Y.PartName,Y.StyleNo

/* Finished Dept PartWise Prod Update */

Union select @guid,y.OrdId,stageid,0,0 as ProdPcs,Seq.sl as Sl,PartName,StyleNo from(select ordid,Mas_Dept.DeptID as id,stageid,sum(ProdPcs) as ProdPcs ,mas_dept.OrderSno,PartName,StyleNo from Trs_Prodentry Mas inner join Trs_ProdentryQty Dtl on MAS.id=Dtl.id Inner Join (Select Distinct Id, PartId From Trs_ProdEntry_SourceStageDtl) Trs_ProdEntry_SourceStageDtl On Trs_ProdEntry_SourceStageDtl.Id = Mas.id left outer join Mas_JobWrkComp M_stage on MAS.StageID =M_stage.Id LEFT OUTER JOIN Mas_Dept ON M_stage.DeptId = Mas_Dept.DeptID LEFT OUTER JOIN Mas_Part ON Trs_ProdEntry_SourceStageDtl.PartId = Mas_Part.PartId where ordid = @Ordid And ISNULL(Mas_Dept.SEMIFINISH,'S') = 'F' group by Mas_Dept.Deptid,stageid,ordid,Mas_Dept.OrderSno,PartName,StyleNo) Y left outer join OrdSeq seq(nolock) on Y.ordid=seq.ordid  and Y.id=seq.Prs where id not in (select deptid from Tmp_OCRSummary_Pcs where guid=@guid )  group by stageid,Y.ordid,Seq.sl,Y.PartName,Y.StyleNo 



 ) X Group By X.GuiId,X.ordid,X.TargetStageID,X.sl,X.PartName,X.StyleNo order by X.sl      



/* Finished Dept PartWise GrnPcs Update */


update Tmp_OCRSummary_Pcs set RecKgs = x.RecPcs from (
Select OrdJob as ordid ,Trs_PcsGrn1.TargetStageID as TargetStageID,Case When (CntColid) > 1 Then  sum(IsNull(RecPcs,0)) Else sum(IsNull(RecPcs,0)) * PcsPerPart End as RecPcs,trs_pcsgrn2.StyleNo,PartName from trs_pcsgrn1 Inner Join trs_pcsgrn2 on trs_pcsgrn1.id=trs_pcsgrn2.id Inner Join (Select Distinct Id , Styleno, ColId, PartId, DCStageId, SizeId From Trs_PcsGrn4_PackingDCDet) Trs_PcsGrn4 On Trs_PcsGrn4.Id = Trs_PcsGrn1.ID And Trs_PcsGrn2.StyleNo = Trs_PcsGrn4.Styleno And Trs_PcsGrn1.TargetStageID = Trs_PcsGrn4.DCStageId And Trs_PcsGrn2.SizID = Trs_PcsGrn4.SizeId
Left Outer Join Mas_Part on Trs_PcsGrn4.PartId=Mas_Part.PartId Inner Join (Select OrdID, StyleNo, PartID, PcsPerPart From Order_PartDtl ) Order_PartDtl On Order_PartDtl.OrdID = Trs_PcsGrn1.OrdJob And Order_PartDtl.StyleNo = Trs_PcsGrn2.StyleNo And Order_PartDtl.PartID = Trs_PcsGrn4.PARTID Inner Join (Select Distinct Id , Styleno,  PartId, COUNT(CntColid) As CntColid  From (Select Distinct Id , Styleno, ColId, PartId, ColId CntColid From Trs_PcsGrn4_PackingDCDet)Y Group By Id , Styleno,  PartId) Z On Z.Id = Trs_PcsGrn1.ID And Z.Styleno = Trs_PcsGrn2.StyleNo And Trs_PcsGrn4.PartId = Z.PartId  where GrnType ='Process Receipt' And ProcessType='P' and OrdJob = @Ordid Group by OrdJob,Trs_PcsGrn1.TargetStageID,trs_pcsgrn2.StyleNo,PartName,PcsPerPart,CntColid )X inner join Tmp_OCRSummary_Pcs tmp on x.ordid =tmp.ordid and x.TargetStageID=tmp.deptid And x.StyleNo=tmp.StyleNo And x.PartName=tmp.PartName and tmp.Guid=@guid 
...
update tmp set tmp.ReworkProdPcs=ProdPcs from    (select X.Ordid,Deptid,stageid,Min(ProdPcs) as ProdPcs,X.StyleNo,X.PartName from(select MAS.Ordid, Mas_Dept.Deptid,stageid,sum(ProdPcs) as ProdPcs,StyleNo,PartName from Trs_Prodentry MAs inner join Trs_ProdentryQty Dtl on MAS.id=Dtl.id left outer join Mas_JobWrkComp M_stage on MAS.StageID =M_stage.Id  LEFT OUTER JOIN Mas_Dept ON M_stage.DeptId = Mas_Dept.DeptID  left outer join Mas_Part on MAS.PartId=Mas_Part.PartId where MAS.ordid=@ORdid and  Mas.Rework=1  group by Mas_Dept.Deptid,stageid,MAS.Ordid,StyleNo,PartName,Mas_Dept.OrderSno) X group by stageid,X.Ordid,X.StyleNo,X.PartName,X.deptid)Y inner join Tmp_OCRSummary_Pcs tmp on Y.ordid=tmp.ordid and Y.stageid=tmp.deptid And Y.StyleNo=tmp.StyleNo And Y.PartName=tmp.PartName and tmp.Guid=@guid  left outer join OrdSeq seq(nolock) on Y.ordid=seq.ordid  and Y.deptid=seq.Prs 

/* For FinDept PartWise Production */


update tmp set tmp.InhouseProdPcs=ProdPcs from    (select X.Ordid,Deptid,stageid,Min(ProdPcs) as ProdPcs,X.StyleNo,X.PartName from(select MAS.Ordid, Mas_Dept.Deptid,stageid,sum(ProdPcs) * PcsPerPart as ProdPcs,Mas.StyleNo,PartName from Trs_Prodentry MAs inner join Trs_ProdentryQty Dtl on MAS.id=Dtl.id Inner Join (Select Distinct Id, PartId From Trs_ProdEntry_SourceStageDtl) Trs_ProdEntry_SourceStageDtl On Trs_ProdEntry_SourceStageDtl.Id = Mas.id  left outer join Mas_JobWrkComp M_stage on MAS.StageID =M_stage.Id  LEFT OUTER JOIN Mas_Dept ON M_stage.DeptId = Mas_Dept.DeptID  left outer join Mas_Part on Trs_ProdEntry_SourceStageDtl.PartId=Mas_Part.PartId Inner Join (Select OrdID, StyleNo, PartID, PcsPerPart From Order_PartDtl ) Order_PartDtl On Order_PartDtl.OrdID = MAs.OrdId And Order_PartDtl.StyleNo = MAs.StyleNo And Order_PartDtl.PartID = Trs_ProdEntry_SourceStageDtl.PARTID where MAS.ordid=@ORdid and  Mas.Rework=0 And ISNULL(Mas_Dept.SEMIFINISH,'S') = 'F'  group by Mas_Dept.Deptid,stageid,MAS.Ordid,Mas.StyleNo,PartName,Mas_Dept.OrderSno,PcsPerPart) X group by stageid,X.Ordid,X.StyleNo,X.PartName,X.deptid)Y inner join Tmp_OCRSummary_Pcs tmp on Y.ordid=tmp.ordid and Y.stageid=tmp.deptid And Y.StyleNo=tmp.StyleNo And Y.PartName=tmp.PartName and tmp.Guid=@guid  left outer join OrdSeq seq(nolock) on Y.ordid=seq.ordid  and Y.deptid=seq.Prs 

/* Rework ProdPcs Update */

update tmp set tmp.ReworkProdPcs=ProdPcs from    (select X.Ordid,Deptid,stageid,Min(ProdPcs) as ProdPcs,X.StyleNo,X.PartName from(select MAS.Ordid, Mas_Dept.Deptid,stageid,sum(ProdPcs) * PcsPerPart as ProdPcs,Mas.StyleNo,PartName from Trs_Prodentry MAs inner join Trs_ProdentryQty Dtl on MAS.id=Dtl.id Inner Join (Select Distinct Id, PartId From Trs_ProdEntry_SourceStageDtl) Trs_ProdEntry_SourceStageDtl On Trs_ProdEntry_SourceStageDtl.Id = Mas.id  left outer join Mas_JobWrkComp M_stage on MAS.StageID =M_stage.Id  LEFT OUTER JOIN Mas_Dept ON M_stage.DeptId = Mas_Dept.DeptID  left outer join Mas_Part on Trs_ProdEntry_SourceStageDtl.PartId=Mas_Part.PartId  Inner Join (Select OrdID, StyleNo, PartID, PcsPerPart From Order_PartDtl ) Order_PartDtl On Order_PartDtl.OrdID = MAs.OrdId And Order_PartDtl.StyleNo = MAs.StyleNo And Order_PartDtl.PartID = Trs_ProdEntry_SourceStageDtl.PARTID  where MAS.ordid=@ORdid and  Mas.Rework=1 And ISNULL(Mas_Dept.SEMIFINISH,'S') = 'F'  group by Mas_Dept.Deptid,stageid,MAS.Ordid,Mas.StyleNo,PartName,Mas_Dept.OrderSno,PcsPerPart) X group by stageid,X.Ordid,X.StyleNo,X.PartName,X.deptid)Y inner join Tmp_OCRSummary_Pcs tmp on Y.ordid=tmp.ordid and Y.stageid=tmp.deptid And Y.StyleNo=tmp.StyleNo And Y.PartName=tmp.PartName and tmp.Guid=@guid  left outer join OrdSeq seq(nolock) on Y.ordid=seq.ordid  and Y.deptid=seq.Prs 
...
update Tmp_OCRSummary_Pcs set RetKgs = x.RecPcs from     (select OrdJob as ordid ,trs_pcs1.TargetStageID, sum(IsNull(RecPcs,0)) * PcsPerPart as RecPcs,Dtl.StyleNo,PartName from trs_pcsgrn1 MAs (nolock)  inner join trs_pcsgrn2 dtl(nolock)  on MAs.id=dtl.id  Inner Join (Select Distinct Id, Styleno, PartId, SizeId, DCStageId, DCId From Trs_PcsGrn4_PackingDCDet) As Trs_PcsGrn4 On Trs_PcsGrn4.Id = MAs.ID And dtl.StyleNo = Trs_PcsGrn4.Styleno And Trs_PcsGrn4.SizeId = dtl.SizID inner join trs_pcs1 on trs_pcs1.Id = Trs_PcsGrn4.DCId And Trs_Pcs1.TargetStageID = Trs_PcsGrn4.DCStageId  left outer join Mas_Part on Trs_PcsGrn4.PartId=Mas_Part.PartId  Inner Join (Select OrdID, StyleNo, PartID, PcsPerPart From Order_PartDtl ) Order_PartDtl On Order_PartDtl.OrdID = MAs.OrdJob And Order_PartDtl.StyleNo = dtl.StyleNo And Order_PartDtl.PartID = Trs_PcsGrn4.PARTID  where GrnType ='Process Return' And MAs.ProcessType='P' and Ordjob =@Ordid group by OrdJob ,trs_pcs1.TargetStageID,Dtl.StyleNo,PartName, PcsPerPart ) x inner join Tmp_OCRSummary_Pcs tmp on x.ordid=tmp.ordid and x.TargetStageID=tmp.deptid And x.StyleNo=tmp.StyleNo And x.PartName=tmp.PartName and tmp.Guid=@guid    


update Tmp_OCRSummary_Pcs set RRetKgs = x.RecPcs from     (select OrdJob as ordid ,Trs_Pcs1.TargetStageID, sum(IsNull(RecPcs,0)) * PcsPerPart as RecPcs,Dtl.StyleNo,PartName from trs_pcsgrn1 MAs (nolock)  inner join trs_pcsgrn2 dtl(nolock)  on MAs.id=dtl.id   Inner Join (Select Distinct Id, Styleno, PartId, SizeId, DCStageId, DCId From Trs_PcsGrn4_PackingDCDet) As Trs_PcsGrn4 On Trs_PcsGrn4.Id = MAs.ID And dtl.StyleNo = Trs_PcsGrn4.Styleno And Trs_PcsGrn4.SizeId = dtl.SizID inner join trs_pcs1 on trs_pcs1.Id = Trs_PcsGrn4.DCId And Trs_Pcs1.TargetStageID = Trs_PcsGrn4.DCStageId  left outer join Mas_Part on Trs_PcsGrn4.PartId=Mas_Part.PartId  Inner Join (Select OrdID, StyleNo, PartID, PcsPerPart From Order_PartDtl ) Order_PartDtl On Order_PartDtl.OrdID = MAs.OrdJob And Order_PartDtl.StyleNo = dtl.StyleNo And Order_PartDtl.PartID = Trs_PcsGrn4.PARTID  where GrnType ='Process Return' And MAs.ProcessType='R' and Ordjob =@Ordid group by OrdJob ,Trs_Pcs1.TargetStageID,Dtl.StyleNo,PartName, PcsPerPart ) x inner join Tmp_OCRSummary_Pcs tmp on x.ordid =tmp.ordid and x.TargetStageID=tmp.deptid And x.StyleNo=tmp.StyleNo And x.PartName=tmp.PartName and tmp.Guid=@guid    
...
update tmp set tmp.GRNQty= x.REcQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(REckgs) as REcQty from trs_grn1 MAs inner join TRs_grn2 dtl on MAs.id=Dtl.id inner join Stocktable Stk on Dtl.Stockid=stk.stockid  where Grntype='Acc.Purch' and
...
update tmp set tmp.Retkgs= x.REcQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(REckgs) as REcQty from trs_grn1 MAs inner join TRs_grn2 dtl on MAs.id=Dtl.id inner join  Stocktable Stk on Dtl.Stockid=stk.stockid  where Grntype='Acc.Iss.Ret' 
...
update Tmp_OCR_AccSummary set poretkgs = x.accpokgs from   (select dtl.OrdID as ORdid ,PRs_dePt,StockTable.Atype,StockTable.Ades,ColId,siz,sum(Kg) as accpokgs from trs_del1 MAs(nolock) inner join  trS_del2 dtl(nolock) on MAs.id=Dtl.id INNER JOIN StockTable ON StockTable.StockID=dtl.stockid where trtype=6 and dtl.OrdID= @Ordid group by dtl.OrdID  ,PRs_dePt,StockTable.Atype,StockTable.Ades,ColId,siz)X  INNER JOIN Tmp_OCR_AccSummary ON Tmp_OCR_AccSummary.Ordid=X.ORdid and Tmp_OCR_AccSummary.AccTypeId=X.Atype and  Tmp_OCR_AccSummary.AccdescId=X.Ades And Tmp_OCR_AccSummary.Colid = X.ColID And Tmp_OCR_AccSummary.Sizeid = X.Siz Where Tmp_OCR_AccSummary.OrdId=@OrdId
...
update tmp set tmp.IssQty= x.IssQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(kg) as IssQty from trs_del1 MAs inner join TRs_del2 dtl on MAs.id=Dtl.id inner join Stocktable Stk on Dtl.Stockid=stk.stockid  where Trtype =7 and   Dtl.ordid =
...
update tmp set tmp.PrsIssQty= x.PrsIssQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(kg) as PrsIssQty from trs_del1 MAs inner join TRs_del2 dtl on MAs.id=Dtl.id inner join Stocktable Stk on Dtl.Stockid=stk.stockid  where Trtype =10 and   Dtl.ordid =
@Ordid group by Dtl.Ordid,Atype,Ades,stk.colid,Siz ) x inner join     Tmp_OCR_AccSummary Tmp on X.ordid=Tmp.ordid and X.Atype=Tmp.AccTypeId and X.Ades=Tmp.AccdescId and X.clr=Tmp.colid and X.siz=Tmp.sizeid and tmp.Guid=@guid  
...
update tmp set tmp.PrsIssRetQty= x.PrsIssRetQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(REckgs) as PrsIssRetQty from trs_grn1 MAs inner join TRs_grn2 dtl on MAs.id=Dtl.id inner join  Stocktable Stk on Dtl.Stockid=stk.stockid  where Grntype='Acc.Proc.Return' 
and  Dtl.ordid =@Ordid  group by Dtl.Ordid,Atype,Ades,stk.colid,Siz ) x inner join   Tmp_OCR_AccSummary Tmp on X.ordid=Tmp.ordid and X.Atype=Tmp.AccTypeId and X.Ades=Tmp.AccdescId and X.clr=Tmp.colid and X.siz=Tmp.sizeid and tmp.Guid=@guid  
