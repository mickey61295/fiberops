...
/* ---yarn details */    
 ...
 select @guid,1,'YARN DETAILS',@ordid,pro_reqyarn2.deptid,deptname,'' as fabid,pro_reqyarn2.countid,pro_reqyarn2.colid ,pro_reqyarn.Reqkgs as qty    
  ,pro_reqyarn.Reqkgs * pro_reqyarn2.Rate as amt from pro_reqyarn2(nolock) inner join MAs_dept(nolock) on pro_reqyarn2.deptid=Mas_dept.deptid inner join  pro_reqyarn (nolock) on pro_reqyarn2.ordid=pro_reqyarn.ordid and pro_reqyarn2.deptid=  
...
SUM((CASE WHEN Mas_UOM_1.UOM = 'KGS' THEN dbo.Pro_ReqKnitt.ReqKgs ELSE dbo.Pro_ReqKnitt.ReqMtr END) * (ISNULL(dbo.Pro_ReqKnitt2.Rate, 0)  + ISNULL(dbo.Pro_ReqKnitt2.AddRate, 0))) AS Amount from pro_reqknitt2(nolock) inner join MAs_dept(nolock) on   
pro_reqknitt2.deptid=Mas_dept.deptid     
INNER JOIN dbo.Pro_ReqKnitt(nolock) ON dbo.Pro_ReqKnitt.OrdId = dbo.Pro_ReqKnitt2.OrdId AND dbo.Pro_ReqKnitt.DeptId = dbo.Pro_ReqKnitt2.DeptId AND  dbo.Pro_ReqKnitt.FabId = dbo.Pro_ReqKnitt2.FabId AND dbo.Pro_ReqKnitt.ColId = dbo.Pro_ReqKnitt2.ColId AND  
  
dbo.Pro_ReqKnitt.CntID = dbo.Pro_ReqKnitt2.CntID AND dbo.Pro_ReqKnitt.GSM = dbo.Pro_ReqKnitt2.GSM AND  dbo.Pro_ReqKnitt.GG = dbo.Pro_ReqKnitt2.GG AND dbo.Pro_ReqKnitt.LL = dbo.Pro_ReqKnitt2.LL AND  dbo.Pro_ReqKnitt.DiaID = dbo.Pro_ReqKnitt2.DiaID   and dbo.Pro_ReqKnitt.FinDiaID = dbo.Pro_ReqKnitt2.FinDiaID LEFT OUTER JOIN Mas_UOM as Mas_UOM_1 ON Pro_ReqKnitt2.RateUOM = Mas_UOM_1.UOMID      
...
update b set b.ActualQty= a.kgs ,b.actualamt=a.netamount  from     
(select ordid,dept,fabid,cntid,ColId,sum(kgs) as kgs,sum(netamount) as netamount from trs_billrate group by ordid,dept,ColId,fabid,cntid ) a    
...
/*--Fabric except  MAs_dept.RateMethod <> 'Color' */    
 ...
 insert into  Temp_BudgetAndActual_Det (Guid ,Slno ,Type ,Ordid ,Deptid ,Deptname,colorid ,BudgetQty,BudgetAmt )    
select @guid,2,'FABRIC DETAILS',@ordid,Pro_ReqKnitt2.deptid,deptname,Pro_ReqKnitt2.colid ,SUM(CASE WHEN Mas_UOM_1.UOM = 'KGS' THEN dbo.Pro_ReqKnitt.ReqKgs ELSE dbo.Pro_ReqKnitt.ReqMtr END) as budgetqty,  SUM((CASE WHEN Mas_UOM_1.UOM = 'KGS' THEN dbo.Pro_ReqKnitt.ReqKgs ELSE dbo.Pro_ReqKnitt.ReqMtr END) * (ISNULL(dbo.Pro_ReqKnitt2.Rate, 0)  + ISNULL(dbo.Pro_ReqKnitt2.AddRate, 0))) AS Amount from pro_reqknitt2(nolock) inner join MAs_dept(nolock) on pro_reqknitt2.deptid=Mas_dept.deptid     
INNER JOIN dbo.Pro_ReqKnitt(nolock) ON dbo.Pro_ReqKnitt.OrdId = dbo.Pro_ReqKnitt2.OrdId AND dbo.Pro_ReqKnitt.DeptId = dbo.Pro_ReqKnitt2.DeptId AND  dbo.Pro_ReqKnitt.FabId = dbo.Pro_ReqKnitt2.FabId AND dbo.Pro_ReqKnitt.ColId = dbo.Pro_ReqKnitt2.ColId AND  
  
dbo.Pro_ReqKnitt.CntID = dbo.Pro_ReqKnitt2.CntID AND dbo.Pro_ReqKnitt.GSM = dbo.Pro_ReqKnitt2.GSM AND  dbo.Pro_ReqKnitt.GG = dbo.Pro_ReqKnitt2.GG AND dbo.Pro_ReqKnitt.LL = dbo.Pro_ReqKnitt2.LL AND  dbo.Pro_ReqKnitt.DiaID = dbo.Pro_ReqKnitt2.DiaID     
and dbo.Pro_ReqKnitt.FinDiaID = dbo.Pro_ReqKnitt2.FinDiaID LEFT OUTER JOIN Mas_UOM as Mas_UOM_1 ON Pro_ReqKnitt2.RateUOM = Mas_UOM_1.UOMID     
    where Pro_ReqKnitt.ordid=@ordid and OutputType ='F' and MAs_dept.RateMethod ='Colour' group by Pro_ReqKnitt2.deptid,deptname,Pro_ReqKnitt2.colid    
    
update b set b.ActualQty= a.kgs  ,b.actualamt=a.netamount from    
(select ordid,dept,ColId,sum(kgs) as kgs,sum(netamount) as netamount from trs_billrate group by ordid,dept,ColId) a    
...
(select trs_opening.ordid,trs_opening.dept,ColId,sum(kgs) as kgs,sum(kgs *trs_opening.rate) as netamount from trs_opening inner join stocktable on trs_opening.stockid= stocktable.stockid  group by trs_opening.ordid,trs_opening.dept,ColId) a    
  inner join Temp_BudgetAndActual_Det  b on a.ordid=b.ordid and a.dept=b.deptid   and a.ColId =b.colorid   inner join MAs_dept as c(nolock)  on b.deptid=c.deptid where  b.ordid=@ordid and OutputType ='F'   and c.RateMethod ='Colour'    
    
      
 /* --update transfer in kgs*/    
  update b set b.ActualQty=  b.ActualQty+a.kgs  ,b.actualamt=isnull(b.actualamt,0) + a.netamount from    
(select trs_del2.tranordid as ordid,trs_del1.prs_dept,ColId,sum(kg) as kgs,sum(trs_del2.kg *trs_del2.rate) as netamount from trs_del1 inner join trs_del2 on trs_del1.id=trs_del2.id  inner join stocktable on trs_del2.stockid= stocktable.stockid where trtype=3 and trs_del2.tranordid=@Ordid   group by trs_del2.tranordid,trs_del1.prs_dept,ColId) a    
  inner join Temp_BudgetAndActual_Det  b on a.ordid=b.ordid and a.prs_dept=b.deptid   and a.ColId =b.colorid   inner join MAs_dept as c(nolock)  on b.deptid=c.deptid where  b.ordid=@ordid and OutputType ='F'   and c.RateMethod ='Colour'    
    
        
  /*--update transfer out kgs*/    
  update b set b.ActualQty=  b.ActualQty-a.kgs  ,b.actualamt=isnull(b.actualamt,0) - a.netamount from    
(select trs_del2.ordid as ordid,trs_del1.prs_dept,ColId,sum(kg) as kgs,sum(trs_del2.kg *trs_del2.rate) as netamount from trs_del1 inner join trs_del2 on trs_del1.id=trs_del2.id  inner join stocktable on trs_del2.stockid= stocktable.stockid where trtype=3 
  
and trs_del2.ordid=@Ordid   group by trs_del2.ordid,trs_del1.prs_dept,ColId) a    
  inner join Temp_BudgetAndActual_Det  b on a.ordid=b.ordid and a.prs_dept=b.deptid   and a.ColId =b.colorid   inner join MAs_dept as c(nolock)  on b.deptid=c.deptid where  b.ordid=@ordid and OutputType ='F'   and c.RateMethod ='Colour'    
...
select @guid,3,'ACCESSORY DETAILS',@Ordid,Pro_AccBudRate.prsid,deptname,Pro_AccBudRate.Acc_Type ,Pro_AccBudRate.Acc_Desc ,Pro_AccBudRate.Clr ,Pro_AccBudRate.Siz, SUM(dbo.PRO_AccReq.ReqdQty) AS reqqty , SUM(dbo.PRO_AccReq.ReqdQty * (ISNULL(dbo.Pro_AccBudRate.BudRate, 0) + ISNULL(dbo.Pro_AccBudRate.AddRate, 0))) AS amount FROM dbo.PRO_AccReq INNER JOIN dbo.Pro_AccBudRate ON dbo.PRO_AccReq.OrdID = dbo.Pro_AccBudRate.OrdID AND dbo.PRO_AccReq.Acc_Type = dbo.Pro_AccBudRate.Acc_Type AND dbo.PRO_AccReq.Acc_Desc =
  
 dbo.Pro_AccBudRate.Acc_Desc AND dbo.PRO_AccReq.Clr = dbo.Pro_AccBudRate.Clr AND dbo.PRO_AccReq.Siz = dbo.Pro_AccBudRate.Siz  inner join Mas_dept on Pro_AccBudRate.prsid=MAs_dept.deptid    
 where pro_accreq.ordid=@ordid and Mas_dept.RateMethod ='-'    
  ...
  update b set b.ActualQty= a.kgs ,b.actualamt=a.netamount from     
  (select ordid,dept,atype,Ades,ColId,Asiz,sum(kgs) as kgs,sum(netamount) as netamount from trs_billrate group by ordid,dept,atype,Ades,ColId,Asiz) a    
...
insert into  Temp_BudgetAndActual_Det (Guid ,Slno ,Type ,Ordid ,Deptid ,Deptname,styleno,budgetamt )    
 ...
 /* --Piece form Details*/    
  ...
  select @guid,5,'PCS FORM', ordid ,ProdPros,deptname,styleno, sum(qty) as qty,sum(Qty*rate) from Pro_ProdPros inner join MAs_dept (nolock) on Pro_ProdPros.ProdPros                =MAs_dept.DeptID  where ordid=@ORdid and  rate >0 group by ordid ,ProdPros,
...
deptname,styleno    
   ...
   Begin
       update b set b.ActualQty= a.kgs ,b.actualamt=a.netamount from     
    (select ordid,departmentid as Dept,styleno,IsNull(Sum(Y.ProdPcs),0) as kgs,sum(Wages) as netamount from Wages_ProductionMas X INNER JOIN Wages_ProductionDet Y ON X.MasSlno = Y.DetSlno group by ordid,departmentID,styleno) a    
     inner join Temp_BudgetAndActual_Det  b on a.ordid=b.ordid and a.dept=b.deptid and a.styleno=b.styleno  inner join MAs_dept as c(nolock)  on b.deptid=c.deptid where               b.ordid=@ordid and slno=5    
   End
    
 /* --update commercial Details*/    
  select @OrdQty= isnull(sum(orderqty),0)  from orderqtydtl where ordid=@Ordid     
    
   select @OrdExsQty = isnull(sum(CutPlanQty ),0)  from orderqtydtl where ordid=@Ordid     
 ...
 else  case when pers_perpcs='P' then  @OrdQty*Val else   case when pers_perpcs ='D' then Total  end  end     end as BudgetAmt   from PRo_BudCommercial INNER JOIN                dbo.Mas_Commercial ON dbo.PRo_BudCommercial.ComID = dbo.Mas_Commercial.ID    
 where ordid=@Ordid    
    
 update a set a.Actualamt=  x.billamt     
 from     
 (select ShippingBill_det.ordid ,comid ,sum(BillAmount) as billamt  from ShippingBill (nolock) inner join ShippingBill_det(nolock) on ShippingBill.id =ShippingBill_det.cid  where ShippingBill_det.ordid=@ordid group by ShippingBill_det.ordid ,comid )x inner join Temp_BudgetAndActual_Det a on x.ordid=a.ordid and x.comid=a.comid  where a.ordid=@Ordid and guid=@guid    
 ...
 update Temp_BudgetAndActual_Det set ActualQty =@Despatchqty where ordid=@Ordid and guid=@guid and slno=6 and isnull(ActualQty ,0)=0    
  ...
  (select  trs_expenses.ordid ,expid ,isnull(sum(Amount),0) as billamt  from trs_expenses  where trs_expenses.ordid=@Ordid  group by trs_expenses.ordid ,expid) x inner join Temp_BudgetAndActual_Det a on x.ordid=a.ordid and x.expid=a.comid  where a.ordid= 
 
  @ordid  and guid=@guid and billamt>0    
  ...
  ordid,deptid,deptname ,fabid,Cntid ,ColId ,Atype ,ades ,Asiz ,trs_billrate.Kgs ,trs_billrate.netamount     
   ...
   @ProcessBud numeric(18,3),    
    ...
    @BudComm numeric(18,3),    
      ...
      Select @ExpSalesbud= ISNULL(Sum(OrderQtyDtl.OrderQty * SaleRate * isnull(Crate,0)),0)  from OrderMas Inner Join OrderQtyDtl on OrderMas.OrdId = OrderQtyDtl.OrdID Where OrderMas.Ordid= @ordid    
    
   SELECT    @ExpSalesAct= ISNULL(SUM(Trs_Pcs2.Pcs * Trs_Pcs2.Rate * Case When IsNull(OrderMas.ActCRate,0) = 0 then ISNULL(Trs_Pcs2.Crate, 0)  Else OrderMas.ActCRate End ), 0)  FROM  Trs_Pcs1 INNER JOIN Trs_Pcs2 ON Trs_Pcs1.ID = Trs_Pcs2.ID INNER JOIN OrderMas INNER JOIN OrderQtyDtl ON OrderMas.OrdId = OrderQtyDtl.OrdID ON Trs_Pcs1.Ordjobno = OrderMas.OrdId AND Trs_Pcs2.StyleNo = OrderQtyDtl.StyleNo AND Trs_Pcs2.ColID = OrderQtyDtl.ColID AND Trs_Pcs2.SizeID = OrderQtyDtl.SizeId AND Trs_Pcs2.StyleID = OrderQtyDtl.StyleID WHERE     (OrderMas.OrdId = @Ordid ) AND (Trs_Pcs1.DelType = 'Despatch')    
...
CommericalAct ,DescBud,DescAct, DebitValueAct,Stockvalue,DirDebitval,Dircreditval, FabricBudgetAmt,FabricActAmt,AccBudgetAmt,AccActAmt,PrdnBudgetAmt,PrdnActAmt ) values (@guid,@ExpSalesBud ,@ExpSalesAct ,@ProcessBud ,@ProcessAct ,@DDBBud ,@DDBAct ,@ProdBud ,@ProdAct ,@SalesCommBud , @SalesCommAct ,@CommBud , @CommAct , @DescBud ,   
@DescAct ,@DebitValueAct ,@Stockvalue ,@dirdebval ,@dircreval,@FabricBudAmt,@FabricActAmt,@AccBudAmt,@AccActAmt,@PrdnBudAmt,@PrdnActAmt )    
set nocount off    
end
