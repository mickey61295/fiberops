...
; Description  :  Order History Panel - Balu      
...
; Last Change Date :  15/06/2024 11.30 AM  
...
CREATE PROCEDURE sp_iohistorypanelright (@Ipaddr as varchar(25))  
...
declare curs1 cursor for  select coycode ,dcno ,ordid  ,deptname ,pname ,ProcessType ,TrsType ,grnno ,GrnFinyear ,partydcref ,GItemDesc  ,RecBags ,RecKgs ,Recmtr ,RUOM ,RecMainUom ,deptid ,grndate ,PurType,dcfinyear ,RecDia,External_GRN,SubProcess,ColorDesc,CompDescr,CompID  from TempIohisRight where IpAddress=@ipaddr order by grndate,grnno  
...
Declare @CompDescr Char(200)
                              
Declare @CompID int


fetch next from curs1 into       @coycode,@dcno,@ordid ,@deptname,@pname ,@processtype,@TrsType,@grnno,@grnfinyear,@partydcref,@gitemdesc,@recbags,@reckgs,@recmtr,@ruom,@RecMainUom,@deptid,@grndate,@purtype,@dcfinyear ,@recdia,@MultiGRN,@SubProcess,@ColorDesc,@CompDescr,@CompID   
...
if @deptname='ACCESSORIES' 
	...
	ELSE
	 ...
	 select top 1 @cnt=sl1 from TempIoHisLedger where ordid = @ordid And deptname = @deptname And pname = @pname And grnno Is null And DcPro_TYpe = @ProcessType And ipaddress = @Ipaddr and CompDescr=@CompDescr and CompID=@CompID  order by sl1  
...
Insert into TempIoHisLedger (Coycode,Pname,Deptname,DeptId,OrdId,GTrsType,Grnno,Grnfinyear,GrnDate,PDcRef,GItemDesc,RecBags,RecKgs,Recmtr,RUom,PurType,RecMainUom,DcPro_TYpe,IPAddress,recdia,External_GRN,SubProcess,ColorDesc,CompDescr,CompID) values(@COYCODE,@Pname,@Deptname,@DeptId,@OrdId,@TrsType,@Grnno,@grnfinyear,@GrnDate,@PARTYDCREF,@gitemdesc,@recbags,@reckgs,@recmtr,@RUom,@pURTYPE,@recmainuom,@ProcessType,@ipaddr,@recdia,@MultiGRN,@SubProcess,@ColorDesc,@CompDescr,@CompID)  
...
select top 1 @cnt=sl1 from TempIoHisLedger where ordid = @ordid And deptname = @deptname And pname = @pname And grnno Is null And dcpro_type = @ProcessType And ipaddress = @Ipaddr and DCNo=@dcno and dcfinyear=@dcfinyear and CompDescr=@CompDescr and CompID=@CompID  
...
Insert into TempIoHisLedger (Coycode,Pname,Deptname,DeptId,OrdId,GTrsType,Grnno,Grnfinyear,GrnDate,PDcRef,GItemDesc,RecBags,RecKgs,Recmtr,RUom,PurType,RecMainUom,DcPro_TYpe,IPAddress,dcno,dcfinyear,recdia,External_GRN,SubProcess,ColorDesc,CompDescr,CompID)   values(@COYCODE,@Pname,@Deptname,@DeptId,@OrdId,@TrsType,@Grnno,@grnfinyear,@GrnDate,@PARTYDCREF,@gitemdesc,@recbags,@reckgs,@recmtr,@RUom,@pURTYPE,@recmainuom,@ProcessType,@ipaddr,@dcno,@dcfinyear,@recdia,@MultiGRN,@SubProcess,@ColorDesc,@CompDescr,@CompID)  
