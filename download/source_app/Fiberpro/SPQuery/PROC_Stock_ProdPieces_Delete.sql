...
; Last Change Date :  01/02/2025 10.50 AM 

; =============================================  */  



CREATE Procedure PROC_Stock_ProdPieces_Delete (@Id int,@sizeId int,@ProdPcs Int) AS  DECLARE @Coycode Int,@Ordid Int,@StyleNo Varchar(20),@StageId Int,@PartId Int,@GodId Int,@ColId Int,@StockQty Int,@SourceStageId Int,@FinalStage Char(1),@SeqNo int,@PartyId Int,@PcsStockId Int,@Rework Int,@RejectionTypeId Int ,@SqlCond as Varchar(100),@LotID int  ,@PcsPerColor int,@LotNo Varchar(30)   ,@LotRequired char(1)  



Select @Id=@Id     Select  @Coycode = CoyId From Trs_ProdEntry Where Id=@Id      select @PartyId = 0      

SELECT @Ordid = OrdId From Trs_ProdEntry Where Id=@Id      SELECT @StyleNo = StyleNo From Trs_ProdEntry Where Id=@Id      SELECT @LotID = IsNull(LotID,0) From Trs_ProdEntry Where Id=@Id      SELECT @LotNo = LotName from Mas_Lot Where LotSNo = @LotID     

SELECT @Stageid = StageId From Trs_ProdEntry Where Id=@Id       SELECT @PartId = PartId From Trs_ProdEntry Where Id=@Id     SELECT @GodId = GodId From Trs_ProdEntry Where Id=@Id     SELECT @Rework = Rework From Trs_ProdEntry Where Id=@Id      

SELECT @RejectionTypeId = RejectionTypeId From Trs_ProdEntry Where Id=@Id      Select @SeqNo = SeqNo From Trs_ProdEntry Inner Join Prod_Sequence On Trs_ProdEntry.OrdId=Prod_Sequence.OrdId And Trs_ProdEntry.StyleNo=Prod_Sequence.StyleNo And Trs_ProdEntry.StageId=Prod_Sequence.StageId Where Id=@Id     SELECT @FinalStage = Mas_Dept.SemiFinish From Trs_ProdEntry Inner Join Mas_JobWrkComp On Trs_ProdEntry.StageId=Mas_JobWrkComp.Id Inner Join Mas_Dept On Mas_JobWrkComp.DeptId=Mas_Dept.DeptId Where Trs_ProdEntry
.Id=@Id      
...
UPDATE Pcs_StockTableQty SET StockQty=Pcs_StockTableQty.StockQty-@ProdPcs,ProductionQty=Pcs_StockTableQty.ProductionQty-@ProdPcs From Pcs_StockTableQty Inner Join Pcs_StockTable On Pcs_StockTable.PcsStockId=Pcs_StockTableQty.PcsStockId Inner Join Trs_Prodentry On Pcs_StockTable.Coycode=Trs_Prodentry.CoyId And Pcs_StockTable.OrdId=Trs_Prodentry.OrdId And Pcs_StockTable.StyleNo=Trs_Prodentry.StyleNo And
...
@RejectionTypeId End And ISNULL(Pcs_StockTable.EmpID,0) = 0)       
...
@RejectionTypeId End 	And Pcs_StockTableQty.SizeId=@SizeId /*	and IsNull(Pcs_StockTableQty.GoodPcsFlag,'G')=Case When IsNull(@Rework,0)=0 Then 'G' Else 'M' End 	and IsNull(Pcs_StockTableQty.RejectionTypeId,0)=Case When (IsNull(@Rework,0)=0 OR IsNull(@Rework,0)=2 )Then 0 Else @Rej
ectionTypeId End */ 	WHERE Pcs_StockTable.coycode=Trs_Prodentry.CoyId And Pcs_StockTable.Ordid=Trs_Prodentry.Ordid and Pcs_StockTable.StyleNo=Trs_Prodentry.StyleNo And Pcs_StockTable.LotId=Trs_Prodentry.LotID And Pcs_StockTable.PartId=Trs_Prodentry.PartId
...
=Max(IsNull(PcsStockId,0))+1 From Pcs_StockTable INSERT INTO Pcs_StockTable (Coycode,Ordid,styleNo,Stageid,PartId,SeqNo,GodId,PcsStockId,PartyId,LotID) VALUES (@Coycode,@Ordid,@StyleNo,@SourceStageid,@PartId,@SeqNo,@GodId,@PcsStockId,@PartyId,@LotID)    INSERT INTO Pcs_StockTableQty (PcsStockId,colid,Sizeid,StockQty,GoodPcsFlag,RejectionTypeId) VALUES (@PcsStockId,@ColId,@Sizeid,@StockQty,Case When (IsNull(@Rework,0)=0 OR IsNull(@Rework,0)=2) Then 'G' Else 'M' End,Case When (IsNull(@Rework,0)=0 OR IsNull(@Rework,0)=2 ) Then 0 Else @RejectionTypeId End)   
...
Ordid=@Ordid and StyleNo=@StyleNo and  GodId=@GodId and PartyId=@PartyId and Trs_ProdEntry_SourceStageDtl.ID  = @ID   And ISNULL(Pcs_StockTable.EmpID,0) = 0
...
Begin     /* UPDATE Pcs_StockTableQty SET StockQty=Pcs_StockTableQty.StockQty+(@ProdPcs*@PcsPerColor) From Pcs_StockTableQty Inner Join Pcs_StockTable On Pcs_StockTable.PcsStockId=Pcs_StockTableQty.PcsStockId Inner Join Trs_Prodentry On Pcs_StockTable.Coycode=Trs_Prodentry.CoyId And  Pcs_StockTable.OrdId=Trs_Prodentry.OrdId And Pcs_StockTable.StyleNo=Trs_Prodentry.StyleNo And Pcs_StockTable.LotId=Trs_Prodentry.LotID And Pcs_StockTable.GodId=Trs_Prodentry.GodId And Pcs_StockTableQty.SizeId=@SizeId and IsNull(Pcs_StockTableQty.GoodPcsFlag,'G')=Case When (IsNull(@Rework,0)=0 OR IsNull(@Rework,0)=2) Then 'G' Else 'M' End and IsNull(Pcs_StockTableQty.RejectionTypeId,0)=Case When (IsNull(@Rework,0)=0 OR IsNull(@Rework,0)=2) Then 0 Else @RejectionTypeId End INNER JOIN Trs_ProdEntry_SourceStageDtl ON
...
INSERT INTO Pcs_StockTable (Coycode,Ordid,styleNo,Stageid,PartId,SeqNo,GodId,PcsStockId,PartyId,LotID) VALUES (@Coycode,@Ordid,@StyleNo,@SourceStageid,@PartId,@SeqNo,@GodId,@PcsStockId,@PartyId,@LotID) 
