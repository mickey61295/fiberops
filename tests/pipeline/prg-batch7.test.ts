...
describe('PRG Batch 7 — SPEC-M43 program-flow revival', () => {
  ...
  describe('Source contracts + wiring pins (the M43 truth)', () => {
    ...
    it('the new tools are registered: 257 = 249 at M43 + the M44 CST quartet + the M45/M46 payroll quartet (side_quest)', () => {
      expect(allTools.length).toBe(257) // M44 CST: +create/update/list_cost_component +get_order_cost // M45 L-01: +get_operator_statement // M46 L-02: +create/commit/get_payroll_runs
    ...
    it('menu: 142 items, /programs/propose is LIVE with the propose tool door', () => {
    ...
    it('PROMPT_VERSION is bumped and the prompt teaches the proposal reflex', () => {
      ...
      expect(PROMPT_VERSION.startsWith('m47')).toBe(true) // M47 = the side_quest merge (m44 costing + m46 payroll lines)
    })
