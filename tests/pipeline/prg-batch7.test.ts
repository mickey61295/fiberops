...
describe('PRG Batch 7 — SPEC-M43 program-flow revival', () => {
  ...
  describe('Source contracts + wiring pins (the M43 truth)', () => {
    ...
    it('the new tools are registered: 258 = 249 at M43 + the M44 CST quartet + the M45/M46 payroll quartet (side_quest)', () => {
      expect(allTools.length).toBe(275) // M44 CST // M45 L-01: +get_operator_statement // M46 L-02: +create/commit/get_payroll_runs // M48 L-03: +get_statutory_register
    ...
    it('menu: 147 items, /programs/propose is LIVE with the propose tool door', () => {
      expect(MENU_ITEMS.length).toBe(151) // M60: +login-audit // M59: +admin-hub // M52 M-03 final-accounts quartet // M48 L-03 statutory (+ M46 L-02 payroll)
    ...
    it('PROMPT_VERSION is bumped and the prompt teaches the proposal reflex', () => {
      expect(PROMPT_VERSION).toBe('m61.2-2026-09-15') // M51 M-02 true double-entry posts on the M50 line
      expect(PROMPT_VERSION.startsWith('m61')).toBe(true) // SPEC-M61 H1 (E-2 prompt rules) on the M56 line
    })
