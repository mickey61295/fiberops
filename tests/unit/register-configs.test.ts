...
const ROUTE_BY_SLUG: Record<string, string> = {
  ...
  'statutory': '/hr/statutory', // SPEC-M47 L-03
}

describe('register-configs — SPEC-M4 §4 contracts', () => {
  it('Wave A+B set + M5 Waves A/B + M6 Wave C + M19 Waves A/B/D + M15 audit + M39 jobworker-statement + M42 waste-percent + M45 operator-statement + M46 payroll: exactly the 41 register configs (order-status board is not a RegisterScreen)', () => {
...
describe('delegated read tools — json SHAPES frozen (PITFALLS #25)', () => {
  ...
  it('M5 Wave B tool doors: get_production_wages delegates to the wages register (142 total)', () => {
    ...
    expect(allTools.length).toBe(254) // M47 L-03: +get_statutory_register // M46 L-02: +create_payroll_run +commit_payroll_run +get_payroll_runs // M45 L-01: +get_operator_statement // M43 PRG: +set_order_deliveries +correct_program_spec +propose_program_requirements // M42 INV: +create_stock_take +record_stock_counts +advance_stock_take // 189 + M19-C ×33
  })
})
