 ...
 * Guards the frozen contract: 129 items (113 parity + M9 live-tracker +
 * M11 feature-flags + M19 ×13 registers + tally-export + M13 digest), 17 groups, unique ids/routes,
...
describe('menu registry — frozen contract (SPEC-M1)', () => {
  it('has exactly 129 items (113 parity + M9 live-tracker + M11 feature-flags + M19 ×13 + tally + M13 digest)', () => {
    expect(MENU_ITEMS.length).toBe(129)
  })
  ...
  it('parityStats: 129/129 live after M13 (113 parity M6 + live-tracker + feature-flags + 13 registers + tally + digest)', () => {
    const s = parityStats()
    expect(s.totalItems).toBe(129)
    expect(s.liveItems).toBe(129)
