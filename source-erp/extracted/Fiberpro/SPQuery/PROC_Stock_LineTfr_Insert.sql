...
SELECT @EMPID = TOEMPID from Trs_LineTfr as trs_pcs1 where id=@id      
SELECT @FRM_EMPID = EMPID from Trs_LineTfr as trs_pcs1 where id=@id      
...
END    
	SELECT @DelType = '' from Trs_LineTfr as Trs_Pcs1 Where id =@Id     
		...
		ELSE 	  
			...
			BEGIN	  
				SELECT @PartyID = 0   
			END /*Select Top 1 @FinishedStageID = StageId  From Pcs_StockTable A INNER JOIN Mas_JobWrkComp B ON A.StageId = B.ID INNER JOIN Mas_Dept C ON B.DeptId = C.DeptID INNER JOIN Trs_Pcs1 D ON A.ORdid = D.Ordjobno Where D.ID = @ID And StyleNo=@StyleNo And  SEMIFINISH='F'   */ 
BEGIN    
...
End 
	...
	If EXISTS (select * from Pcs_StockTable Inner Join Pcs_StockTableQty On Pcs_StockTable.PcsStockId=Pcs_StockTableQty.PcsStockId where coycode=@coycode and Ordid=@Ordid and StyleNo=@StyleNo And LotId = @LotID and Stageid=@FinishedStageID and PartId=@PartId
		 and GodId=@GodId and PartyId=0  and Pcs_StockTableQty.ColId=@Colid and Pcs_StockTableQty.SizeId=@SizeId and ISNULL(Pcs_StockTable.EmpID,0) = @FRM_EMPID and IsNull(GoodPcsFlag,'G')='G' and IsNull(RejectionTypeId,0)=0 )     
 ...
 End    
  If (Select IsNull(PcsType,'Piece') From Mas_JobWrkComp Where Id=@StageId)='Piece' or (Select IsNull(PcsType,'Piece') From Mas_JobWrkComp Where Id=@StageId)='Bit'  Or (@Stageid=@SourceStageid)        
   Begin      
    Select @PcsStockId=PcsStockId From Pcs_StockTable where coycode=@coycode and Ordid=@Ordid and StyleNo=@StyleNo And LotID = @LotID and Stageid=@SourceStageId and PartId=@PartId and GodId=@GodId and PartyId=0 And ISNULL(EmpID,0) = @FRM_EMPID 
...
print 'g111111'
  ...
  Update Pcs_StockTableQty Set Pcs_StockTableQty.RewrkStk=IsNull(Pcs_StockTableQty.RewrkStk,0)-@StockQty From Pcs_StockTableQty Inner Join Pcs_StockTable On Pcs_StockTable.PcsStockId=Pcs_StockTableQty.PcsStockId where coycode=@coycode and Ordid=@Ordid and
 StyleNo=@StyleNo and LotId = @LotID and Stageid=@SourceStageId and PartId=@PartId and GodId=@GodId and PartyId=0 and Pcs_StockTableQty.ColId=@Colid and Pcs_StockTableQty.SizeId=@SizeId and ISNULL(Pcs_StockTable.EmpID,0) = 0 and IsNull(GoodPcsFlag,'G')=Case When @ProcessType='P' Then 'G' Else 
 'M' End and IsNull(RejectionTypeId,0)=Case When @ProcessType='P' Then 0 Else @RejectionTypeId  End    
 end
 END
 ELSE
  BEGIN
