...
BEGIN
	SET @GAN_RewrkFlg ='Y'
END
SELECT @EMPID = EMPID from Trs_LineTfr as trs_pcs1 where id=@id    
SELECT @TO_EmpID = TOEMPID from Trs_LineTfr as trs_pcs1 where id=@id    
if @DelType ='Sales'        
SELECT @Partyid = 0  
 ELSE   
SELECT @Partyid = 0 from Trs_LineTfr as trs_pcs1 where id=@id        /* SELECT Top 1 @FinishedStageID = StageId  From Pcs_StockTable A INNER JO
...
SELECT Id,StyleNo,Colid,PartId,SizeId,IsNull(lotNo,0) LotNo,Pcs,SourceStageId FROM Trs_LineTfr_Det as Trs_Pcs2 Where ID=@Id
    OPEN LINE_CURSOR        
...
If EXISTS (select * from Pcs_StockTable where coycode=@coycode and Ordid=@Ordid and StyleNo=@StyleNo and LotId = @LotId and Stageid=@SourceStageid and PartId=@PartId and GodId=@GodId and PartyId=0  AND IsNull(EmpID,0) = @TO_EmpID )     
BEGIN      
...
and IsNull(Pcs_StockTableQty.RejectionTypeId,0)=Case When @ProcessType='P' Then 0 Else @RejectionTypeId End and Partyid=0 And Pcs_StockTable.StageId=@SourceStageId And Trs_Pcs1.Id=@Id 	 AND IsNull(Pcs_StockTable.EmpID,0) = @TO_EmpID 
...
Case When @ProcessType='P' Then 'G' Else 'M' End and IsNull(Pcs_StockTableQty.RejectionTypeId,0)=Case When @ProcessType='P' Then 0 Else @RejectionTypeId End and Partyid=0 And Pcs_StockTable.StageId=@SourceStageId And Trs_Pcs1.Id=@Id  AND IsNull(Pcs_StockTable.EmpID,0) = @EmpID 	
END     
...
INSERT INTO Pcs_StockTableQty (PcsStockId,colid,Sizeid,StockQty,GoodPcsFlag,RejectionTypeId) VALUES (@PcsStockId,@ColId,@Sizeid,@StockQty,Case When @ProcessType='P' Then 'G' Else 'M' End,Case When @ProcessType='P' Then 0 Else @RejectionTypeId End)  	  
 ...
 ELSE    
  ...
  print 'a9'
	SELECT @PcsStockId=PcsStockId From Pcs_StockTable where coycode=@coycode and Ordid=@Ordid and StyleNo=@StyleNo And LotID = @Lotid and Stageid=@Stageid and PartId=@PartId and GodId=@GodId and PartyId=0   AND IsNull(EmpID,0) = @TO_EmpID 	    
	If EXISTS (select * from Pcs_StockTable Inner Join Pcs_StockTableQty On Pcs_StockTable.PcsStockId=Pcs_StockTableQty.PcsStockId where coycode=@coycode and Ordid=@Ordid and StyleNo=@StyleNo And LotID = @LotID and Stageid=@SourceStageid and PartId=@PartId and GodId=@GodId and PartyId=0 and Pcs_StockTableQty.ColId=@Colid and Pcs_StockTableQty.SizeId=@SizeId AND IsNull(Pcs_StockTable.EmpID,0) = @TO_EmpID  and IsNull(GoodPcsFlag,'G')= 'G' and IsNull(RejectionTypeId,0)=0)      
