...
; Change Person    :  ASLAM
; Last Change Date :  25/04/2023 10.25 AM 
; =============================================  */  
CREATE PROCEDURE SP_Vue_Prod_PCSNew (@DBName Varchar(100)) AS 
BEGIN 
...
SELECT Coycode,OrdID,StyleNo,PartID,ColId,SizeId,0 as CutPcs,Sum(LinePcs) as LineFeedPcs,Sum(LineOutPcs) AS LineOutputPcs, Sum(GoodPcs) as GoodPcs,Sum(RejPcs) as RejectPcs, Sum(ReworkPcs) as reWorkWIP  FROM (
Select Coycode,Final_StageId as StageId,b.MasterOrderID as OrdID,A.StyleNo,A.PartID,LineID,count(C.ActualPcs) as Linepcs, 0 as LineOutPcs,0 as GoodPcs,0 as RejPcs, 0 as ReworkPcs,A.ColorID as ColID,C.SizeId From ' + @DBName  + '..Cutting A INNER JOIN ' + @DBName  + '..Orders B ON A.OrderID = B.OrderID INNER JOIN ' + @DBName  + '..Bundle C ON 
 ...
 Select Coycode,Final_StageId as StageId,b.MasterOrderID as OrdID,A.StyleNo,A.PartID,LineID,0 as Linepcs, count(Prod.BundlePieceID) as LineOutPcs,0 as GoodPcs,0 as RejPcs, 0 as ReworkPcs,A.ColorID as ColID,C.SizeId From ' + @DBName  + '..Cutting A INNER JOIN ' + @DBName  + '..Orders B ON A.OrderID = B.OrderID INNER JOIN ' + @DBName  + '..Bundle C ON 
...
UNION
 ...
 Select Coycode,Final_StageId as StageId,b.MasterOrderID as OrdID,A.StyleNo,A.PartID,LineID,0 as Linepcs, 0 as LineOutPcs,COUNT(Prod.BundlePieceID) as GoodPcs,0 as RejPcs, 0 as ReworkPcs,A.ColorID as ColID,C.SizeId From ' + @DBName  + '..Cutting A INNER JOIN ' + @DBName  + '..Orders B ON A.OrderID = B.OrderID INNER JOIN ' + @DBName  + '..Bundle C ON 
 ...
 INNER JOIN ' + @DBName  + '..LineIssue ON LineIssue.LineIssueID = Li.LineIssueID  
 INNER JOIN Mas_Exporter on A.Coycode = Mas_Exporter.ExpID 
 WHERE Prod.EntryType =''GD''
 GROUP BY Coycode,b.MasterOrderID,A.StyleNo,A.PartID,LineID,Final_StageId,A.ColorID ,C.SizeId

 UNION
 Select Coycode,Final_StageId as StageId,b.MasterOrderID as OrdID,A.StyleNo,A.PartID,LineID,0 as Linepcs, 0 as LineOutPcs,0 as GoodPcs,COUNT(Prod.BundlePieceID) as RejPcs, 0 as ReworkPcs,A.ColorID as ColID,C.SizeId From ' + @DBName  + '..Cutting A INNER JOIN ' + @DBName  + '..Orders B ON A.OrderID = B.OrderID INNER JOIN ' + @DBName  + '..Bundle C ON 
 A.CuttingID = C.CuttingID INNER JOIN ' + @DBName  + '..BundlePiece D ON C.BundleID = D.BundleID INNER JOIN ' + @DBName  + ' ..ProductionEntry Prod ON 
 D.BundlePieceID = prod.BundlePieceID inner join ' + @DBName  + '..LineIssueEntry LI on li.BundleID = C.BundleID                                 
 INNER JOIN ' + @DBName  + '..LineIssue ON LineIssue.LineIssueID = Li.LineIssueID  
 INNER JOIN Mas_Exporter on A.Coycode = Mas_Exporter.ExpID 
 ...
 UNION
 Select Coycode,Final_StageId as StageId,b.MasterOrderID as OrdID,A.StyleNo,A.PartID,LineID,0 as Linepcs, 0 as LineOutPcs,0 as GoodPcs,0 as RejPcs, COUNT(Prod.BundlePieceID) as ReworkPcs,A.ColorID as ColID,C.SizeId From ' + @DBName  + '..Cutting A INNER JOIN ' + @DBName  + '..Orders B ON A.OrderID = B.OrderID INNER JOIN ' + @DBName  + '..Bundle C ON 
 A.CuttingID = C.CuttingID INNER JOIN ' + @DBName  + '..BundlePiece D ON C.BundleID = D.BundleID INNER JOIN ' + @DBName  + ' ..ProductionEntry Prod ON 
 D.BundlePieceID = prod.BundlePieceID inner join ' + @DBName  + '..LineIssueEntry LI on li.BundleID = C.BundleID                                 
 INNER JOIN ' + @DBName  + '..LineIssue ON LineIssue.LineIssueID = Li.LineIssueID  
 INNER JOIN Mas_Exporter on A.Coycode = Mas_Exporter.ExpID 
 WHERE Prod.EntryType =''RW''
 GROUP BY Coycode,b.MasterOrderID,A.StyleNo,A.PartID,LineID,Final_StageId,A.ColorID ,C.SizeId ) Y GROUP BY Coycode,OrdID,StyleNo,PartID,ColId,SizeId'
