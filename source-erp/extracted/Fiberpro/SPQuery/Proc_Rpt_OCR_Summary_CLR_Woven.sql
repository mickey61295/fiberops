...
; Last Change Date :  25-02-2025 10.45 AM
; ============================================= */    
CREATE PROCEDURE Proc_Rpt_OCR_Summary_CLR_Woven(@Guid varchar(256),@Ordid int)    
...
Insert into Tmp_OCRSummary (guid,Ordid,Deptid,ReqKgs,DeptSlno,Color) select @guid,knitt.Ordid,deptid, Sum(ReqMtr) as ReqKgs ,seq.sl,'' as ColorDesc  from Pro_Reqknitt knitt (nolock) left outer join OrdSeq seq(nolock) on knitt.ordid=seq.ordid and knitt.DeptId=seq.Prs  where knitt.ordid=@Ordid and DeptId in (1,2,3,4) group by  knitt.Ordid,deptid, seq.sl  order by sl 


Insert into Tmp_OCRSummary (guid,Ordid,Deptid,ReqKgs,DeptSlno,Color) select @guid,knitt.Ordid,deptid, Sum(ReqMtr) as ReqKgs ,seq.sl, isnull(ColorDesc,'')  as ColorDesc  from Pro_Reqknitt knitt (nolock) left outer join OrdSeq seq(nolock) on knitt.ordid=seq.ordid and knitt.DeptId=seq.Prs LEFT OUTER JOIN Mas_Color ON Mas_Color.ColID = knitt.ColId where knitt.ordid=@Ordid and DeptId not in (1,2,3,4) group by  knitt.Ordid,deptid, seq.sl ,ColorDesc order by sl 
...
update Tmp_OCRSummary set DelKgs = x.DelKgs from   (select dtl.ordid,Prs_dept,sum(mtr) as DelKgs,'' as ColorDesc from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID left outer join Mas_Color on Mas_Color.ColID = StockTable.ColID where trtype=1 And ProcessType='P' and Prs_Dept in (2,3,4) and dtl.Ordid= @Ordid group by dtl.Ordid,Prs_dept union all select dtl.ordid,Prs_dept,sum(mtr) as DelKgs,isnull(ColorDesc,'') as ColorDesc from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID left outer join Mas_Color on Mas_Color.ColID = StockTable.ColID where trtype=1 And ProcessType='P' and Prs_Dept not in (2,3,4,38) and dtl.Ordid= @Ordid group by dtl.Ordid,Prs_dept,ColorDesc union all select dtl.ordid,Prs_dept,sum(mtr) as DelKgs,isnull(ColorDesc,'') as ColorDesc from trs_del1 MAs(nolock)  inner join Trs_del2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID left outer join Mas_Color on Mas_Color.ColID = MAs.DyeColId where trtype=1 And ProcessType='P' and Prs_Dept=38 and dtl.Ordid= @Ordid group by dtl.Ordid,Prs_dept,ColorDesc  )x   inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid and x.Prs_dept=tmp.deptid and x.ColorDesc=tmp.Color and tmp.Guid=@guid     
...
update tmp set RecKgs = x.RecKgs from    (select dtl.ordid,Mas.Dept,sum(Recmtr) as RecKgs,'' as ColorDesc from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID left outer join Mas_Color on Mas_Color.ColID = StockTable.ColID  where grntype in ('Process' ,'Purchase') And ISNULL(processtype,'P')='P' and dtl.Ordid= @Ordid and MAs.Dept in (2,3,4) group by dtl.Ordid,Mas.Dept union all select dtl.ordid,Mas.Dept,sum(Recmtr) as RecKgs,isnull(ColorDesc,'') as ColorDesc from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID left outer join Mas_Color on Mas_Color.ColID = StockTable.ColID  where grntype in ('Process' ,'Purchase') And ISNULL(processtype,'P')='P' and dtl.Ordid= @Ordid and MAs.Dept not in (2,3,4) group by dtl.Ordid,Mas.Dept,ColorDesc )x inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid   and x.Dept=tmp.deptid and x.ColorDesc=tmp.Color and tmp.Guid=@guid  
...
update tmp set Retkgs = x.RecKgs from     (select dtl.ordid,MAs.Dept,sum(Recmtr) as RecKgs,'' as ColorDesc from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID left outer join Mas_Color on Mas_Color.ColID = StockTable.ColID  where grntype in ('Process Return') and dtl.Ordid= @Ordid and MAs.Dept in (2,3,4) group by dtl.Ordid,MAs.Dept union all select dtl.ordid,MAs.Dept,sum(Recmtr) as RecKgs,isnull(ColorDesc,'') as ColorDesc from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID left outer join Mas_Color on Mas_Color.ColID = StockTable.ColID  where grntype in ('Process Return') and dtl.Ordid= @Ordid and MAs.Dept not in (2,3,4,38) group by dtl.Ordid,MAs.Dept,ColorDesc union all select dtl.ordid,MAs.Dept,sum(Recmtr) as RecKgs,isnull(ColorDesc,'') as ColorDesc from trs_grn1 MAs(nolock)  inner join Trs_grn2 dtl (nolock) on Mas.id=Dtl.id inner join StockTable (nolock) on StockTable.StockID = dtl.StockID and dtl.OrdId = StockTable.OrdID INNER JOIN Trs_Del1 ON Trs_Del1.ID = MAs.DCID left outer join Mas_Color on Mas_Color.ColID = Trs_Del1.DyeColId  where grntype in ('Process Return') and dtl.Ordid= @Ordid and MAs.Dept  in (38) group by dtl.Ordid,MAs.Dept,ColorDesc )x inner join Tmp_OCRSummary tmp on x.ordid=tmp.ordid  and x.Dept=tmp.deptid and x.ColorDesc=tmp.Color and tmp.Guid=@guid     
...
Update Tmp Set ShortageKgs=X.ShortKgs From(Select OrdID,Dept,Sum(ShortMtr) as ShortKgs,'' as ColorDesc From Trs_Shortage left outer join Mas_Color on Mas_Color.ColID = Trs_Shortage.ColID Where Trs_Shortage.Dept in (2,3,4) Group by OrdID,Dept union all Select OrdID,Dept,Sum(ShortMtr) as ShortKgs,isnull(ColorDesc,'') as ColorDesc From Trs_Shortage left outer join Mas_Color on Mas_Color.ColID = Trs_Shortage.ColID Where Dept Not In (0,2,3,4,16,17) Group by OrdID,Dept,ColorDesc)X INNER JOIN Tmp_OCRSummary Tmp ON  X.Ordid=Tmp.Ordid and X.Dept=Tmp.Deptid and x.ColorDesc=tmp.Color and Tmp.Guid=@guid  
...
update tmp set tmp.Retkgs= x.REcQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(Recmtr) as REcQty from trs_grn1 MAs inner join TRs_grn2 dtl on MAs.id=Dtl.id inner join  Stocktable Stk on Dtl.Stockid=stk.stockid  where Grntype='Acc.Iss.Ret' and  Dtl.ordid =@Ordid  group by Dtl.Ordid,Atype,Ades,stk.colid,Siz ) x inner join   Tmp_OCR_AccSummary Tmp on X.ordid=Tmp.ordid and X.Atype=Tmp.AccTypeId and X.Ades=Tmp.AccdescId and X.clr=Tmp.colid and X.siz=Tmp.sizeid and tmp.Guid=@guid  
...
update tmp set tmp.PrsIssQty= x.PrsIssQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(mtr) as PrsIssQty from trs_del1 MAs inner join TRs_del2 dtl on MAs.id=Dtl.id inner join Stocktable Stk on Dtl.Stockid=stk.stockid  where Trtype =10 and   Dtl.ordid =@Ordid  group by Dtl.Ordid,Atype,Ades,stk.colid,Siz ) x inner join     Tmp_OCR_AccSummary Tmp on X.ordid=Tmp.ordid and X.Atype=Tmp.AccTypeId and X.Ades=Tmp.AccdescId and X.clr=Tmp.colid and X.siz=Tmp.sizeid and tmp.Guid=@guid    

/* Process Return */

update tmp set tmp.PrsIssRetQty= x.PrsIssRetQty from (Select Dtl.Ordid,Atype,Ades,Stk.ColID as Clr,Siz,Sum(Recmtr) as PrsIssRetQty from trs_grn1 MAs inner join TRs_grn2 dtl on MAs.id=Dtl.id inner join  Stocktable Stk on Dtl.Stockid=stk.stockid  where Grntype='Acc.Proc.Return' and  Dtl.ordid =@Ordid  group by Dtl.Ordid,Atype,Ades,stk.colid,Siz ) x inner join   Tmp_OCR_AccSummary Tmp on X.ordid=Tmp.ordid and X.Atype=Tmp.AccTypeId and X.Ades=Tmp.AccdescId and X.clr=Tmp.colid and X.siz=Tmp.sizeid and tmp.Guid=@guid 
