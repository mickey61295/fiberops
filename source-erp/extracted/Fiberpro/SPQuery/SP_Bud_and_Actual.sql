...
; Last Change Date :  29/12/2025 09.50 AM
...
CREATE PROCEDURE SP_Bud_and_Actual(@gblcode int,@Guid nvarchar(256),@OrdID nvarchar(4000),@Reqd_TaxInPL char(1),@GPAY char(3) ='YES') as  
...
insert into Temp_BudgetAndActual  (Guid,slno ,ordid,deptid ,budgetqty,budgetamt,processsno) SELECT @Guid,1  ,AR.OrdID ,16  ,
SUM(AR.ReqdQty) ,SUM(AR.ReqdQty * (ISNULL(ABR.BudRate, 0) /* + ISNULL(ABR.AddRate, 0)*/ )),2  FROM  PRO_AccReq AR  (nolock)
...
INNER JOIN OrderMas ON AR.Ordid =OrderMas.Ordid WHERE AR.ordid in(select id from fnSplitter(@OrdID)) And PurchaseType='Purchase'
   group by AR.ordid
...
--group by AR.ordid
 ...
 SUM(AR.ReqdQty * (ISNULL(ABR.BudRate, 0) /* + ISNULL(ABR.AddRate, 0) */ )) ,2   FROM  PRO_AccReq AR  (nolock)    
 ...
 INNER JOIN OrderMas ON AR.Ordid = OrderMas.Ordid INNER JOIN Mas_Dept ON Mas_dEpt.DeptID=Ar.PrsID WHERE 
AR.ordid in(select id from fnSplitter(@OrdID))  And AR.ItemType='S' And PurchaseType='Process'  and Mas_Dept.AccProsDept='Y' 
 group by AR.ordid,Ar.PrsID
...
;with   cte(ordid,actqty,actamt,DeptID) as      
(    SELECT Ordid,Sum(Kgs) as KGs, Sum(NetAmount) as NetAmount,Dept FROM (SELECT   TBR.OrdID,   ISNULL(SUM(TBR.Kgs), 0) Kgs ,
...
Case When IsNull(BillType,'')in('Purchase','Process') Then Case When IsNull(Trs_Po1.Fcy,0)>0 Then
 ...
 Case When IsNull(BillType,'')in('Purchase','Process')  Then Case When IsNull(Trs_Po1.Fcy,0)>0 Then   
 Sum(TBR.Amount) *  IsNull(exchangeRate,0) ELSE  Sum(TBR.Amount) END END END As NetAmount ,TBR.dept  FROM        
...
GROUP BY TBR.OrdID, TBR.dept ,IsNull(BillType,''),IsNull(exchangeRate,0) ,IsNull(Trs_Po1.Fcy,0) ) X GROUP BY ORDID,Dept  )    
update tmp    set  ActualQty = actqty, ActualAmt =actamt    from Temp_BudgetAndActual tmp   join cte on  tmp.ordid = cte.ordid    
