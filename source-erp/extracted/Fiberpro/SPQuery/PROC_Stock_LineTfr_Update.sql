...
,@Knit_Woven_Both_OrderType as char(1), @GAN_RewrkFlg Char(1) ='N',@EmpID  INT,@TO_EmpID  INT
...
Select @OldId=@Id    
Select @OldCoycode = Coycode From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId     
select @OldPartyId = 0 From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId     
select @EmpID = isNull(EmpID,0) From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId     
select @TO_EmpID = isNull(ToEmpId,0) From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId     
SELECT @OldOrdid = Ordjobno From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId     
SELECT @LotwiseStockReqd = isNull(LotwiseStock,'Y') from Ordermas2 where Ordid=@OldOrdid  
SELECT @OldStageid = TargetStageId From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId     
SELECT @OldGodId = GodId From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId     
SELECT @ProcessType = 'P' from Trs_LineTfr as trs_pcs1 where id=@oldid     
SELECT @RejectionTypeId = 0 from Trs_LineTfr as trs_pcs1 where id=@oldid    
Select @OldSeqNo = SeqNo From Prod_Sequence Where Prod_Sequence.OrdId=@OrdId And Prod_Sequence.StyleNo=@StyleNo And Prod_Sequence.StageId=@StageId      SELECT @OldFinalStage = Mas_Dept.SemiFinish From Trs_LineTfr as Trs_Pcs1 Inner Join Mas_JobWrkComp On  Trs_Pcs1.TargetStageID=Mas_JobWrkComp.Id Inner Join  Mas_Dept On Mas_JobWrkComp.DeptId=Mas_Dept.DeptId Where Trs_Pcs1.Id=@OldId      
...
SELECT @OldStockQty = Pcs from Trs_LineTfr_Det trs_Pcs2 where Id = @ID and StyleNo = @StyleNo And colId = @ColId and  PartId = @PartID And SizeID = @SizeId And LotNo= @LotNo    
SELECT @OldDelType = '' From Trs_LineTfr as  Trs_Pcs1 Where Id=@OldId      
SELECT @OldBuyerId = 0 From Trs_LineTfr as Trs_Pcs1 Where Id=@OldId       
...
SELECT @FinalStage = Mas_Dept.SemiFinish From Trs_LineTfr as Trs_Pcs1 Inner Join Mas_JobWrkComp On Trs_Pcs1.TargetStageId=Mas_JobWrkComp.Id Inner Join Mas_Dept On Mas_JobWrkComp.DeptId=Mas_Dept.DeptId Where Trs_Pcs1.Id=@Id 
