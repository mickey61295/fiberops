...
; Description      :  QUERY
; Change Person    :  ASLAM
; Last Change Date :  14/08/2023 03.35 PM 
; =============================================  */  

CREATE PROCEDURE SP_Qry21 (@tempStr as nvarchar(max),@tempStr_1 as nvarchar(max),@coycode nvarchar(max)) AS
...
SELECT  Accuom,ExporterName,Pid,DeptId,Deptname, Pname, OrdId, BuyOrdNo, SUM(RBag) AS RBag, SUM(RecKgs) AS RecKgs, SUM(Recmtr) AS Recmtr, RecMethod,  Proddept, docno,finyear,id,dt, LotNo,X.Colid,ISNULL(ColorDesc,'''') As ColorDesc,MultiGrn from ( SELECT ACCUOM.Uom
 as Accuom,Mas_Exporter.ExporterName,mas_party.Pid,Mas_Dept.DeptId,Mas_Dept.Deptname, Mas_Party.Pname, OrderMas.OrdId, OrderMas.BuyOrdNo, SUM(Trs_GRN2.RBag) AS RBag, SUM(Trs_GRN2.RecKgs) AS RecKgs, SUM(Trs_GRN2.Recmtr) AS Recmtr, Mas_dept.RecMethod,IsNUll(Mas_dept.Proddept,''N'') Proddept,trs_del1.docno,trs_del1.finyear,trs_del1.id,trs_del1.dt,Case When IsNull(TRs_Del1.Lotno,'''')<>'''' Then IsNull(TRs_Del1.Lotno,'''') Else IsNull(StockTable.LotNo,'''') End As LotNo,StockTable.ColId, '''' As MultiGrn FROM Trs_Grn1 INNER JOIN Trs_Del1 ON Trs_Grn1.DcId=Trs_Del1.Id INNER JOIN Trs_GRN2 ON Trs_Grn1.ID = Trs_GRN2.ID INNER JOIN StockTable ON Trs_GRN2.StockID = StockTable.StockID INNER JOIN Mas_Exporter ON Trs_Grn1.Coycode = Mas_Exporter.ExpID INNER JOIN Mas_Dept ON Trs_Grn1.Dept 
