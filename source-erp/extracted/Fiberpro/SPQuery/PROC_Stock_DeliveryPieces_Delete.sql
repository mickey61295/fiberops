...
select @Partyid = 0  
 ELSE   
select @Partyid = IsNull(Party,0) from trs_pcs1 where id=@id        /* SELECT Top 1 @FinishedStageID = StageId  From Pcs_StockTable A INNER JO
...
Select Id,StyleNo,Colid,PartId,SizeId,IsNull(lotNo,0) LotNo,Pcs,SourceStageId FROM Trs_Pcs2 Where ID=@Id
    OPEN LINE_CURSOR        
