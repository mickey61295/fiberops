...
const readTools: AgentTool[] = [
  ...
  {
    name: 'get_bundle',
    description: 'Resolve a cut bundle by bundleNo (CUT-####/B#) or barcode (*CUT####B###*) — the scan reflex. Returns bundle, order, style, qty, status and the label-print link. Use when an operator scans or reads a bundle tag.',
    domain: 'cutting',
    isWrite: false,
    schema: z.object({
      bundle: z.string().describe('Bundle number (CUT-0001/B1) or barcode (*CUT0001B001*) or db id.'),
    }),
    async execute(args) {
      const key = args.bundle.trim()
      if (!key) return { text: 'Provide a bundle number (CUT-0001/B1) or barcode (*CUT0001B001*).' }
      let bundle = await db.cutBundle.findUnique({ where: { bundleNo: key } }).catch(() => null)
      ...
      if (!bundle) return { text: `No bundle matches '${key}'. Bundles are generated by cut orders (CUT-####/B#).` }
      const cut = bundle.cutOrderId
        ...
        : null
      return {
        text: `Bundle ${bundle.bundleNo} — order ${cut?.order?.orderNo ?? '—'}, qty ${bundle.qty} pcs, status ${bundle.status}.`,
        json: {
          bundleNo: bundle.bundleNo,
          barcode: bundle.barcode,
          cutNo: cut?.cutNo,
          orderNo: cut?.order?.orderNo,
          style: cut?.order?.style?.styleNo,
          qty: bundle.qty,
          status: bundle.status,
          labelPrint: `/print/bundle-label/${encodeURIComponent(bundle.bundleNo)}`,
        },
      }
    },
  },
