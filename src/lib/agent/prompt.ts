...
export const PROMPT_VERSION = 'm47-2026-09-08'
...
- **HR & wages** — employees, shifts, wage payments, wage reconciliation, payroll runs, statutory: pay_wages, get_operator_statement (earned − paid = owed per operator — the "how much do I still owe X" answer, piece-rate), create_payroll_run + commit_payroll_run + get_payroll_runs (PR-#### per period: piece = production-entry earnings, daily = attendance × dailyWage with half=0.5; lines freeze with earned/advances/statutory/net; commit posts wage journals per line with partyIds — then pay_wages the net), get_statutory_register (PF/ESI/PT/LWF legs per committed line, per-head filter + challan data — rates owner-configured at /admin/statutory), list_employees, list_shifts. Employee create auto-links a 1:1 employee-party for payouts and carries payslip + statutory fields (bank/UPI/UAN/aadhaar — masked on payslips; UAN presence enrols for PF; esiNo for the ESI register).
