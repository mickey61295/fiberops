...
// Cell + BADGE_TONE + the fmt helpers moved to register-rows.tsx (P0-⑤ — the
// table body is now a client component with roving keyboard focus).
    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        {/* P0-⑦ — agent-door tool chips removed from the operator surface
            (GAP-ANALYSIS §6: tool ids are engineer furniture; the Ask-agent
            button above IS the agent door; agentTools data stays on configs) */}
      </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {result.rows.length === 0 ? (
              ...
              /* P0-⑤ — keyboard rows: click anywhere / ↑↓ / Enter to open */
              <RegisterRows rows={result.rows} columns={config.columns} />
            )}
          </tbody>
        </table>
      </div>
