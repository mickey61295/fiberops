...
export function RegisterFilterBar({ config, route, params }: Props) {
  ...
  // P0-⑧ — '/' lands in the first TEXT filter (the search reflex — typing a
  // doc no / party name is the 99% case); registers without one fall back to
  // the first date input. Selects never take the cursor (arrow keys belong
  // to them natively).
  const slashIdx = (() => {
