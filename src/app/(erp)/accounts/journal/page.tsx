 ...
 * record.
 * SPEC-M50 M-01 — each leg shows its CoA code chip (· 1010) when linked;
 * legs resolve by exact name/code and unknown legs are REFUSED at the door.
 */
...
export default async function JournalPage() {
  const recent = await db.journal.findMany({
    ...
    include: { party: true, debitAccountRef: true, creditAccountRef: true },
  })
  const leg = (name: string, ref: { code: string } | null) => (ref ? `${name} · ${ref.code}` : name)
  const rows = recent.map((j) => ({
    id: j.id,
    cells: {
      voucherNo: j.voucherNo,
      voucherType: j.voucherType,
      debitAccount: leg(j.debitAccount, j.debitAccountRef),
      creditAccount: leg(j.creditAccount, j.creditAccountRef),
