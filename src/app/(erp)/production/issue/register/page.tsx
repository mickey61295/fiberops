/**
 * /production/issue/register — line-issue-register register (SPEC-M19 §2 Wave B). RegisterScreen over the
 * REGISTER_SERVICES['line-issue-register'] read path — the SAME service the config's
...
export default async function Page({
  ...
  const config = getRegisterConfig('line-issue-register')!
  const query = parseRegisterQuery(config, params)
  const result = await REGISTER_SERVICES['line-issue-register'](query)
  const { page: _p, ...rest } = params
  return (
    <RegisterScreen
      config={config}
      result={result}
      route="/production/issue/register"
