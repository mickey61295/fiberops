/**
 * /inventory/stock/fabric — fabric-stock register (SPEC-M19 §1). RegisterScreen over the
 * REGISTER_SERVICES['fabric-stock'] read path — the SAME service the config's
...
export default async function Page({
  ...
  const config = getRegisterConfig('fabric-stock')!
  const query = parseRegisterQuery(config, params)
  const result = await REGISTER_SERVICES['fabric-stock'](query)
  const { page: _p, ...rest } = params
  return (
    <RegisterScreen
      config={config}
      result={result}
      route="/inventory/stock/fabric"
