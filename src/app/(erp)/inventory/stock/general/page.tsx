/**
 * /inventory/stock/general — general-stock register (SPEC-M19 §1). RegisterScreen over the
 * REGISTER_SERVICES['general-stock'] read path — the SAME service the config's
...
export default async function Page({
  ...
  const result = await REGISTER_SERVICES['general-stock'](query)
  const { page: _p, ...rest } = params
  return (
    <RegisterScreen
      config={config}
      result={result}
      route="/inventory/stock/general"
