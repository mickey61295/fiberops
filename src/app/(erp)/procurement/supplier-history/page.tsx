/**
 * /procurement/supplier-history — supplier-history register (SPEC-M19 §2 Wave B). RegisterScreen over the
 * REGISTER_SERVICES['supplier-history'] read path — the SAME service the config's
...
export default async function Page({
  ...
  const config = getRegisterConfig('supplier-history')!
  const query = parseRegisterQuery(config, params)
  const result = await REGISTER_SERVICES['supplier-history'](query)
  const { page: _p, ...rest } = params
  return (
    <RegisterScreen
      config={config}
      result={result}
      route="/procurement/supplier-history"
