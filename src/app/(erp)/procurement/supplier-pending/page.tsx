/**
 * /procurement/supplier-pending — supplier-pending register (SPEC-M19 §2 Wave B). RegisterScreen over the
 * REGISTER_SERVICES['supplier-pending'] read path — the SAME service the config's
...
export default async function Page({
  ...
  const config = getRegisterConfig('supplier-pending')!
  const query = parseRegisterQuery(config, params)
  const result = await REGISTER_SERVICES['supplier-pending'](query)
  const { page: _p, ...rest } = params
  return (
    <RegisterScreen
      config={config}
      result={result}
      route="/procurement/supplier-pending"
