/**
 * /pieces/orderwise — orderwise-pcs register (SPEC-M19 §1). RegisterScreen over the
 * REGISTER_SERVICES['orderwise-pcs'] read path — the SAME service the config's
...
export default async function Page({
  ...
  const config = getRegisterConfig('orderwise-pcs')!
  const query = parseRegisterQuery(config, params)
  const result = await REGISTER_SERVICES['orderwise-pcs'](query)
  const { page: _p, ...rest } = params
  return (
    <RegisterScreen
      config={config}
      result={result}
      route="/pieces/orderwise"
