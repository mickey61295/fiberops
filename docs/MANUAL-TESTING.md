...
Before manual testing begins, the automated gates below must be green. They were last verified on 2026-09-08 on main (commit 359ae5e, the M56 cheque/PDC lifecycle batch — the §17-3 resolution; the remediation queues are empty and the owner decisions §17-2/§17-4 are recorded, only §17-1 backup target remains open). If any gate fails, stop and report the failure before executing the manual suite.

**Automated gates and their expected results**

| Gate | Command | Expected Result |
|---|---|---|
| Unit / pipeline tests | npx vitest run (in fiberops/) | 79 files, 1636 tests, all passed |
