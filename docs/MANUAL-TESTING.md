...
> Version 1.7 · 2026-09-07 · Build under test: `main @ 43fc765` (M53 Tally both sides; v1.7 adds the export cases AC-17..20 — the both-sides counts grid + the doctrine notes + the JSON download, COUNTED ONCE + the purchase side + the exclusion doors, THE EXPORT DOCTRINE PAIR (a reversed receipt exports with its CN- reversal, net zero), and the GST split ledgers + the agent door) · Environment: development (`http://localhost:3000`)
...
The verification round performed on 2026-09-07 on main (commit 43fc765 — the M53 Tally-both-sides batch, Module M Batch 4: the Tally JSON export rewritten on THE EXPORT DOCTRINE, the M52 GL doctrine applied to the export — every journal row renders exactly once (as its document's voucher or as itself; JV- companions never double-export, fixing the M19 adapter's live double-count where every payment appeared twice: 375 vouchers for the same money), the CN- contra IS the reversal (a cancelled transaction exports WITH its reversal and nets zero — a full-window Tally import converges to the GL truth), both-sides coverage (SupplierBill → Purchase with Input CGST/SGST/IGST splits, DebitNote → Credit Note, Expense → Journal, the GST split replacing the single Output GST ledger), the honesty doors (warnings[]: unlinked-bank legs, no-companion defaults, orphan JV-* fallback, stored-math mismatches, draft/cancelled exclusions REPORTED), and get_tally_export (tools 265→266). The Tally XML option is decision §17-4, deferred pending the owner — JSON stands, and the payload's notes say so honestly) covered the automated gates in full and the live route surface by direct request. Every gate below ran on the M53 tree (the M50/M51/M52/M53 batches are on main). Results are summarized below; manual execution of Sections 4-6 by a human tester remains the open work this guide enables.

**Verification results, 2026-09-07 round**

| Check | Result | Detail |
|---|---|---|
| Vitest suite | PASS | 76 files, 1580 tests passed in 48s (includes industry-chain, payroll L01/L02/L03/L04, accounts M-01 + M-02 + M-03 + M-04, FY hotfix, parity suites) |
| TypeScript (src) | PASS | Zero errors under src/; known legacy errors confined to scripts/ cleanup files |
| Context integrity | PASS | context_check.sh: 606/606 checks, NO DRIFT (the m53 PROMPT_VERSION pin; tools 266, menu 147, routes 183, register configs 35, register services 47) |
| Agent routing (static) | PASS | eval_routing.mjs --static PASS (m53-2026-09-06, registry 258) |
| Route smoke (live) | PASS | route_smoke_m53.sh: 23/23 — the export screen renders (×8 counts tiles, the warnings panel, the doctrine notes, the §17-4 deferral copy) · /api/tally full-window 200 json attachment · THE LIVE DOUBLE-COUNT pin (sales 190 + receipts 178 + payments 9 + journals 0, counts sum == vouchers.length — the 187 companions ride their payments) · the crafted today-window walkthrough (the Purchase voucher + Input IGST, the GST split, COUNTED ONCE, the [CANCELLED]+CN- doctrine pair, the draft/cancelled exclusion doors + warnings) · full revert · the from>to and invalid-date 400 refusals; route_smoke_m48..m52 re-basis covered by the shared services |
| Browser E2E (live) | PASS | The payments form door (CUS001, ₹400, cash, ref 'M53 E2E receipt') → the plan card 'GL legs classify to Cash/Bank [1010] / Sundry Debtors [1110]' → Approve → RCP-0188 + companion JV-RCP-0188 DB-verified (Dr 'Cash/Bank' / Cr 'Acme Corp USA' — the party-name Tally legs) → /accounts/tally-export shows exactly ONE RCP-0188 row (the Receipt with the companion's frozen legs — counted once) → Reverse payment through the view → DB-verified (payment cancelled, companion ACTIVE, CN-RCP-0188 contra with swapped legs, INV-0010 re-derived 'issued') → THE DOCTRINE PAIR live on the export screen (the Receipt + the reversal row with the 'rev RCP-0188' badge, Reversals 1) → zero console/page errors → fully reverted (zero residue; screenshots m53-tally-receipt-row / m53-tally-doctrine-pair / m53-tally-page) |
| Login (live) | PASS | admin@fiberpro.local authenticated via /api/auth/login; session payload correct |
| Git state | PASS | Working tree clean; main carries M50/M51/M52/M53 (the batch pushes ride the PAT protocol — inline URL, token audit before/after, ls-remote verify); side_quest fully merged (0 unmerged commits) |
