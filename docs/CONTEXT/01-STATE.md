...
Last verified: 2026-08-30 (session: m23 — SPEC-M23 SHIPPED (mock e-invoice/e-way — gap-audit Gap D #11 CLOSED: 'not even the v1-promised mock IRN exists' → it exists): SalesInvoice +2 additive nullable columns (irnAckNo, ewbNo — models stay 78) · NEW src/lib/erp/einvoice.ts — DETERMINISTIC offline mock honoring the real workflow's rules: irnTuple = the REAL govt input tuple (seller GSTIN from AppOption print.gstin | buyer Party.gstin | invoiceNo | dd/mm/yyyy | value), mockIrnFor = SHA-256 64-hex (the real IRN format — same invoice ⇒ same IRN), mockAckNo 10-digit, mockEwbNo 12-digit (hash-derived digits, no BigInt — target predates ES2020), planGenerateIrn guards (exists · status ISSUED only · no IRN yet — regeneration = the cancellation workflow, honestly out of scope) + the ₹50,000 e-Way consignment threshold (both sides tested) · tools 225→**226** (generate_einvoice_irn docTool, domain accounting) · FORM DOOR: invoice view 'Generate IRN (mock)' button (issued && !irn) → server action through runCommit — a REAL 14th commit door (the M15 grep contract count moved 13→14, pinned) · PRINT: invoice meta +IRN Ack No +e-Way Bill No rows when stamped · VIEW: stamped-IRN block w/ honest 'No e-Way — consignment ≤ ₹50,000' line · OUT (documented): real signing/QR image (needs a QR lib decision), IRN cancellation, export ERN flow · tests: einvoice.test NEW 10 (determinism, formats, guards, threshold, one-IRN, print rows both sides, tool+count) + pins (tools 226 ×7, docTool 54, schemas 41, doors 14) → **968 vitest** · tsc src/ 0 · eval --static PASS · context_check 540→**545/545** NO DRIFT · NEW route_smoke_m23 **15/15** (button on eligible → stamps → button gone → view rows → print rows both sides → tool) · docs: SPEC-M23 frozen + §5 record, this line, milestone row M23, next-actions #28, worklog Task 33) — (prior: m22 — keypad mode) — SECOND SIX-TASK RUN COMPLETE (M16 + hygiene + M20 + M21 + M22 + M23)
