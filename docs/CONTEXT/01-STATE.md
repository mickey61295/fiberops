...
Last verified: 2026-09-01 (session: m41 — SPEC-M41 SHIPPED (Phase-6B Batch 5 procurement & dispatch closure, PRC-01..08 + PRC-09 deferred per §17-6; SEAMS 1+5 CLOSED: the multi-line GRN (lines[] per PO line addressed by itemType+itemCode, per-line receivedQty, PO status from all-lines coverage math — the HFX-01 multi-line refusal RETIRED, the header-qty path stays for single-line POs), PO amendment (planPoAmend — rate/qty line revisions recompute totals, the [amended …] notes trail, qty-below-received refuses; update_purchase_order tool + /procurement/po/amendments door), purchase return (PRN-#### on the GRN table grnType=purchase_return; per-GRN-line rejectedQty cumulative guard; ledger OUT; the PO untouched — supplier-pending reads bills; optional linked DN- debit note ties PAY-03), the REAL PO approval gate (po_appr flag reads the po Approval row before receiving), the DC lifecycle (planDcTransition — LAD conversion loading→despatched with the LAD- number kept; delivered terminal + deliveredAt; the despatch day-book register /dispatch/register with age + gate-pass columns + csv twin), gendcdays WIRED (digest nonReturn section — returnable-days detection over JW DCs, silent when empty), gate refDocNo VALIDATED against real doc families with startsWith suggestions + the clear door + the MIS 'DCs without a gate pass' recon card, logistics fields (lrNo/transporter/freight/cartons/grossWeightKg through schema+form+print)): loop-closure #1 (3-line PO → 2 GRNs → per-line exact, PO received) + #5 (DC colour/size → delivered → print) GREEN; 1263 vitest (+13 NEW prc-batch5 + inherited pin updates); tsc src 0; eval --static PASS (m41-2026-09-01); context_check 604/604 NO DRIFT (14 pins updated to the M41 truth); route_smoke_m41 17/17 LIVE + browser E2E (Mark delivered click → DB deliveredAt → reverted, zero console errors); tools 243, docTools 65, menu 137, routes 171, models 83, schemas 43, posting 39, doc-configs 42, flags 33, registers 37.
