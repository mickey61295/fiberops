...
Last verified: 2026-08-28 (session: m9-wave-a — LIVE TRACKER (the real-time sibling of the parity tracker, user-requested): `src/lib/erp/live-snapshot.ts` ONE collector (health probe SELECT-1 latency + process uptime/RSS + parityStats() verbatim + 12 flow families today/7d counts via the resolve.ts (db as any)[model] pattern + workload counters + merged 12-event feed docs/approvals/agent-turns) + `/api/live-tracker` GET snapshot + `/api/live-tracker/stream` SSE (3s tick, abort-clean, session-guarded M7-B rule) + `/live` page (SSR first snapshot → client `live-tracker.tsx` SSE-with-polling-fallback hook, hidden-tab pause, parity-tracker visual language) + LIVE_ROUTES +/live (meta page, no group — open to any authed user, topbar breadcrumb + parity-footer link); REGRESSION CAUGHT & FIXED: commit 19fdedc accidentally deleted `src/app/api/upload/route.ts` (83 lines gone in a worklog-only commit) → restored from 19fdedc~1, upload-route tests 7/7 green again; vitest 691→700 (+9 live-snapshot: assessHealth thresholds ×4, registry shape ×3 incl. Prisma-delegate existence — caught real bug grn→gRN, snapshot shape ×2), tsc src/ 0 errors, context_check 375/375 (pins: erp-views 28→29, LIVEROUTES 145→146, +6 file-existence); tools stay 188, models stay 65) — (prior: m8-wave-b — the remaining 15 doc detail families print)
...
| M9 — Real-time observability | Wave A: **live tracker** — `/live` page + live-snapshot collector + SSE stream (3s) + polling fallback `/api/live-tracker` — real-time docs/approvals/agent events, server health, workload, parity block | **Wave A COMPLETE** (`m9-wave-a`): 12 flow families + health probe + 12-event merged feed; SSE→polling degradation + hidden-tab pause; session-guarded; LIVE_ROUTES +/live (meta page); vitest 700, tsc src/ 0, context_check 375/375; ALSO restored `/api/upload` (accidentally deleted by commit 19fdedc) |
...
| ERP view/shell components | **29** (28 M8-era + M9-A live-tracker.tsx) | `ls src/components/erp/*.tsx \| wc -l` |
...
| Live routes (M9-A) | **146**: M6-D 145 + M9-A `/live` (live tracker meta page — no menu group) | LIVE_ROUTES in `src/lib/erp/menu-registry.ts` |
...
| **Total vitest** | **700 passing** (691 M8-era + 9 M9-A live-snapshot: assessHealth ×4 — db-down-wins/latency-250/approvals-50/healthy, registry shape ×3 — 12-families pin + unique keys/models + Prisma-delegate existence (caught real grn→gRN bug), snapshot shape ×2 — parity-mirror + events-cap/order + workload keys + runtime diagnostics) | `npx vitest run` |
...
| API routes | `/api/agent`, `/api/agent/approve`, `/api/erp`, `/api/seed`, `/api/upload` (Wave D §12 rebuild; **M9-A: RESTORED — commit 19fdedc accidentally deleted it**), `/api/route.ts` + `/api/auth/login`, `/api/auth/logout`, `/api/auth/session`, `/api/auth/bootstrap`, `/api/auth/admin/set-password` + **M9-A `/api/live-tracker` (GET snapshot) + `/api/live-tracker/stream` (SSE, 3s tick) — both SESSION-GUARDED** — **M7-B: the 5 ERP route files are SESSION-GUARDED (requireApiSession → 401 JSON; /api/auth/* deliberately open; /api/config left open — server-side FlagsProvider, no client fetchers); M7-C: /api/seed additionally ADMIN-ONLY (403) + /api/auth/admin/set-password admin-role door (403 non-admin)** | ls `src/app/api/` |
