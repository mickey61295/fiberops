...
25. **M20 DONE** (2026-08-30, SPEC-M20): attendance — model (upsert per employee/day) + post_attendance/list_attendance tools (222→224) + /hr/attendance day-book + menu (131/164). 942 vitest, tsc src/ 0, eval --static PASS, context_check 531/531, route_smoke_m20 18/18. Branch hygiene DONE (m9-wave-a-alt + p0-reflex-pack-alt deleted local+origin, SHAs logged in worklog Task 29). **Next candidates (P3 lane continues)**: waste receipt (M21) → keypad-operator mode (M22) → e-invoice/e-way mock IRN (M23) → voice entry (V).
