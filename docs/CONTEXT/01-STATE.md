...
| M18 — Print & Command Fidelity (P1, Waves A+B) | SPEC-M18: order print family (fetchers-order.ts, 21st in PRINT_DOCS) + invoice HSN body + bank/remit strip + masthead phone/email/CIN + ?template=large + dc cost-bearing auto-template + ?copies=3 burst + print-on-save pref + CommandPalette on ⌘K (rights-parity jump bar) + agent rebind ⌘J + paste-into-grid | **COMPLETE** (`m18-print-cmd`): print-fidelity.test 7 NEW; vitest 735/735; tsc src/ 0; eval --static PASS; context_check 426/426 (views 31, print lib 7, families 21, doors 20); route_smoke_m18 15/15; route_smoke_m9 regression 38/38. Wave C (doc-view Cancel/Void/Duplicate + rate memory + change password) spec'd in SPEC-M18 §4, unshipped |
...
| ERP view/shell components | **31** (23 + M6-C lifecycle-form + M6-D approval-queue + M8-A print trio + M9 live-tracker + M17 register-rows + M18 command-palette) | `ls src/components/erp/*.tsx \| wc -l` |
