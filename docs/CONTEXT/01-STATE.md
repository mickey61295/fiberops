...
Last verified: 2026-08-29 (session: m12 — Playwright E2E golden paths, SPEC-M9 §9-P1 item 3: real-browser interaction tests over the 8 golden paths — login, order create (form), order create (agent), PO→GRN, invoice→payment, approval approve, print door, rights denial. ONE command `bash scripts/e2e.sh` (= `npm run test:e2e`): playwright.config.ts boots a DEDICATED dev server on :3100 (reuseExistingServer:false — the system :3000 server serves custom.db and must never be reused) whose boot command copies db/custom.db → db/e2e.db (the ISOLATION contract: the dev DB is never touched; md5-summed before/after by the runner — "isolation check OK" line proves it) with DATABASE_URL=file:…/e2e.db · globalSetup (scripts/e2e_global_setup.ts) seeds through the REAL posting services (ADR-001 — planOrder/planInvoice/planPurchaseOrder commits: order SO-#### + invoice ₹1,050 + PO whose commit auto-submits the pending Approval) + E2E-marked masters (buyer E2E-B/style E2E-S1/colour E2E Red/size E2E M/party E2E-P/yarn E2E-Y/godown E2E-G) + admin password force-set on the COPY + group 'E2E Rights' (rights=['orders']) + merchandiser e2e@fiberpro.local; keys land in tests/e2e/.e2e-state.json; globalTeardown deletes e2e.db + state · 8 spec files / 14 test cases in tests/e2e/ (01-login … 08-rights-denial): every spec logs in through the REAL form (no storageState), the two agent specs (03/06) drive the live GLM with self-sufficient prompts + 240s card timeouts + 1 retry, DB-truth companions query e2e.db via a pinned PrismaClient, zero-defect bar = zero pageerror + zero console-error per page (negative-test 401s declared allowed) · TWO REAL BUGS found & fixed: (1) globalSetup's PrismaClient resolved DATABASE_URL from .env → seeded the DEV db (9 runs leaked; scripts/e2e_cleanup_devdb.ts cleaned it, admin password restored; ROOT FIX = e2e.sh exports DATABASE_URL for the whole playwright tree + setup refuses to run without it — PITFALLS #35); (2) /api/agent SSE stream threw "Controller is already closed" when the browser navigated away mid-stream (every enqueue/close on a disconnected controller throws) — fixed with the send()/safeClose() disconnect guard + loop break, so a gone client stops burning LLM steps (PITFALLS #36) · RESULT: 14/14 passed in ~52s, exit 0, isolation verified; @playwright/test@1.62.1 devDep (chromium v1234 already cached — zero download) · gates: tsc src/ 0 · vitest 724/724 · eval_routing --static PASS · route_smoke_m9 38/38 · next build EXIT 0 · context_check 410→418/418 NO DRIFT (+8 m12 checks: config/spec files 8/test cases 14/setup+teardown+runner 3/isolation guard/test:e2e script/SSE guard/SPEC-M12.md; +SPEC-M12.md in file-existence) · curl smokes stay as the cheap gates (nothing migrated) · .gitignore: db/e2e.db*, tests/e2e/.e2e-state.json, tests/e2e/.results/ · docs: SPEC-M12 frozen (+§8 implementation record), this STATE line, milestone row M12, next-actions #13, PITFALLS #35/#36, worklog Task 15) — (prior: m11 — the Feature-flags admin screen)
