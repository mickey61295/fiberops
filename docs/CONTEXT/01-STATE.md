...
Last verified: 2026-08-31 (session: m38 — SPEC-M38 SHIPPED (Phase-6B Batch 2 agent QoL — the 12 CHAT FRs from docs/PRD/PHASE-6B-REMEDIATION-SPEC.md §5; ALL THREE OWNER CHATBOT ISSUES NOW CLOSED: issue 2 conversation-unfriendliness + issue 3 screen-awareness, on top of M36's issue 1): CHAT-01 outcome events (approve/reject/failure append synthetic user-role [Plan … APPROVED/REJECTED] events that render as system chips AND ride the history — the model finally SEES what happened to its plans, prompt §4.2d/§6 satisfiable) + CHAT-02 dynamic context line (src/lib/agent/context.ts: today IST + weekday, user, activeFinYear(), screen title + docNo, godown roster — route.ts appends it after SYSTEM_PROMPT; 'what day is it / which screen am I on' answers correctly LIVE) + CHAT-03 screen-aware suggestions (panel usePathname → findItemByRoute → the 76 authored agentPrompts consumable; [id] routes resolve via parent item; empty state reads 'FROM ORDER REGISTER' LIVE; defaults fixed to real code formats B-####/STY-####) + CHAT-04 follow-up chips (TOOL_FOLLOWUPS map over the screen's agentTools after read answers, LIVE) + CHAT-05 plan contents table (plan-display.ts: field/value rows ₹ en-IN money, dates, line rollup +moreLines; approving shows contents not 'Creates: N record(s)', LIVE) + CHAT-06 approve-by-id kills TOCTOU (tool-call-end carries turnId; approve route loads the STORED toolName+args+plan, re-plans, deep-compares day-normalized — identical → commit, drift → 409 re-review; update({id:turnId}) replaces updateMany-all-turns; panel intercepts typed approve/ok/reject phrases — 'approve it' LIVE resolved the pending plan with zero model round-trips) + CHAT-07 post-commit CTA row (approve response gains docNo + cta{viewUrl,printUrl} from doc-cta.ts prefix map → 21 families; View/Print buttons on the outcome event, LIVE) + CHAT-08 truthful badges (ToolResult.error declared; docTool + masterCreate/Update + failures carry it — failed writes show the red error chip, LIVE on Y-0001-not-found) + CHAT-09 fuzzy lookup rescue (src/lib/erp/lookup.ts resolveByNameOrCode w/ score floor 50 = ask-don't-guess + topCandidates + didYouMean; wired at posting/order.ts buyer+style seams; ambiguous 'Acme ZZZQQQ' → 'Did you mean: B001 (Acme Corp USA)?' LIVE + unit-pinned) + CHAT-10 bounded master lists (boundedList helper: q + take default 20 cap 100 on 13 list tools, text reports total+truncated; route.ts boundedToolContent trims rows + marks truncated:true instead of byte-amputating JSON) + CHAT-11 prompt formatting contract (§9: markdown allowed, ≤h3, tables for comparisons, ₹ en-IN, read answers ≤8 lines; hardcoded FY/G1-G3 prose REPLACED by the [CONTEXT] line; PROMPT_VERSION m38-2026-08-31, eval --static PASS) + CHAT-12 polish sweep (composer autofocus LIVE, copy-message button, aborted chips → 'stopped', dead SSE events consumed as documented no-ops, MAX_STEPS exhaustion → visible error event, list_orders.buyerId honored via RegisterQuery.buyerId (was accepted-and-ignored since M3), humanized tool labels 'Adjust stock'/'Party ledger' + composite keys fixing live duplicate-key console errors) · gates: 1188 vitest (+35) · tsc src/ 0 · eval --static PASS · context_check 587→595/595 NO DRIFT · route_smoke_batch2 NEW 18/18 (429-skip-aware) · LIVE browser-verified: FROM-ORDER-REGISTER suggestions, date+screen answer, plan contents table ₹140 rate row, outcome event + View CTA, typed-approve interceptor, autofocus, zero key errors
Last verified: 2026-08-31 (session: m37 — SPEC-M37 SHIPPED (Phase-6B Batch 1 ops foundation — the 5 OPS from docs/PRD/PHASE-6B-REMEDIATION-SPEC.md §4; trust tier part 2: every number recoverable + every commit once + every day the IST day): OPS-01 backup_db.py (VACUUM INTO snapshot + integrity + 7d/30w rotation + --verify restore-verify + rsync off-box via AppOption ops.backup.rsync_target seeded on first run; install_backup_cron.sh idempotent CRON_TZ=Asia/Kolkata nightly 02:30 + monthly verify; db/backups/ gitignored) + recovery_drill.sh REWRITTEN (newest snapshot → integrity → temp-DB verify → swap; --accept-data-loss GONE from the recovery path; dead seed_stages call dropped) + digest gains the Ops & data-growth section (DB MB · StockLedger/AuditLog/AgentTurn counts · newest backup + age, red when missing/stale) rendered as a card on /notifications/digest · OPS-02 WAL (db.ts runs PRAGMA journal_mode=WAL once per process; live custom.db was 'delete' → now 'wal'; VACUUM INTO snapshots consistent under it) · OPS-03 NEW src/lib/erp/dates.ts (istToday/istDateStr/istDayStart/istTodayDate/istDayStartInstant/dateOrIstToday/endOfUtcDay — fixed +5:30 arithmetic, NO process-TZ flip: that would off-by-one ~100 UTC-midnight display sites via every new Date(y,m,d) constructor; the spec's own pin '23:30 UTC = next-day IST' passes arithmetically) + 46 `? new Date(args.X) : new Date()` fallbacks swept across ALL 24 posting services (scripts/ops3_ist_sweep.py + repair pass) + attendance dayStart + production line-output default + production-bill period/journal date + agent tools today + digest header + gate window (istDayStartInstant — the REAL IST-midnight instant for timestamp columns) + registers/resolve endOfUtcDay + registers/attendance IST default window + CSV filenames (server+client) + bundle-label print fallback; dateOrIstToday propagates NaN so per-service invalid-date validation still fires · OPS-04 NEW IdempotencyKey model (models 78→79) + runCommit meta.idempotencyKey INSERT-FIRST lock (key row = the lock; result stored on success; row DELETED on failure so retry can attempt; audit only by the winner) wired to BOTH doors: commitDocAction(slug,payload,idempotencyKey?) — DocScreen mints the token per reviewed plan in save() — and /api/agent/approve — the panel mints it per pending-approval CARD (route short-circuits replays pre-plan, returns replayed:true; LIVE-verified: two identical approve POSTs → ONE row ADJ-0001, second response replayed:true same id/docNo, test data reverted) · OPS-05 StockLedger.docKey String? @unique (docNo itself can NEVER be unique — GT/PT/RTC/RSP pairs share it across out+in legs, GRN/cut/despatch lines across N rows; docKey = doc-level anchor on exactly ONE row/doc: ADJ/OPN/WST single row, pairs the OUT leg; postLedger gained docKey; racing same-number commits now fail the index INSIDE the commit transaction → 'ADJ-0007 was just taken by another user — retry' via docKeyViolation; live data backfilled 142 anchors = 138 GT + 4 RSP out-legs, zero true duplicates found so no renumbering) · tests: ops-batch1.test.ts NEW 22 (spec pins: 23:30UTC→next IST day + the 18:30Z boundary; replay same-row-id posts-once; concurrent same-key one winner one loud loser; failed commit releases key; same-number collision fails loudly + no dup rows; GT out-leg-only docKey; WAL live pragma; drill has no executable --accept-data-loss; digest ops section) + 2 suites updated for honest IST windows (attendance fixture day = istToday(), was the server-local UTC date) → **1153 vitest** (1131+22) · tsc src/ 0 · eval --static PASS 15/15 · context_check 581→**587/587** NO DRIFT (+SPEC-M37.md +dates.ts +ops-batch1.test.ts +backup_db.py +install_backup_cron.sh +route_smoke_batch1.sh; models 79) · NEW route_smoke_batch1 **15/15** (9 screens incl. money/ledger-family/digest/attendance; digest ops+backup status; custom.db journal_mode=wal; docKey index + IdempotencyKey table live; backfill 142) · LIVE browser-verified (digest Ops & data-growth card: DB 5.4 MB · StockLedger 1,181 · AuditLog 7 · AgentTurn 428 · Backup custom-20260831-004319.db <1h old; screenshot scripts/m37-digest-ops-card.png; lucides-react typo caught by the gate + '&lt;1' entity-in-JSX-expression fix). Historical: 2026-08-31 (session: m36 — SPEC-M36 SHIPPED (Phase-6B Batch 0 hotfix — 19 HFX from docs/PRD/PHASE-6B-REMEDIATION-SPEC.md §3; the trust one-liners + the agent render stack that closes owner issue 1): CORRECTNESS ×13 — HFX-01 planGrn guard (multi-line PO refused naming PRC-01; cancelled/completed refused quoting status) · HFX-02 DC lines persist colourId/sizeId (name→id resolution, packing-list precedent; view/print already rendered them) · HFX-03 cancelled invoices leave party-ledger (both fns) + bills-register (three screens one balance) · HFX-04 only direction:'in' settles an invoice (invoiceRecon + outstanding-summary settledByInvoice — a tagged out-payment never reduces AR) · HFX-05 partyReceipts CONSUMED (on-account receipts reduce AR outstanding, oldest bucket first, clamped at 0) · HFX-06 rtgs+neft join both mode selects (schema-comment contract) · HFX-07 Party master gains partyType=employee (the wage picker filtered to a type nothing could produce — empty forever) · HFX-08 journal sideEffects honest ('Cash/bank balance updated' was false — no GL) · HFX-09 'billed' ghost dead (filter option + JOBWORK_STATUS until JWL-06) · HFX-10 PO complete writes 'received' (PO_STATUS has no 'completed' — enum drift since M6) · HFX-11 NEW src/lib/erp/valuation.ts shared valueBucket() per-uom valuation (dashboard + stock-register call it — the WAC seam) · HFX-12 shiftWages readers → amount (the column has NO writer): daily-pnl wages = Σ amount AND produced = qty × order contract rate (margin = the spread — both non-zero for any day with production); budget shiftWages field reads amount with the double-count addend dropped (identical on live data); production-status + operation-summary wage columns follow · HFX-13 vitest pinned OFF the production DB (globalSetup copies custom.db→test.db per run, setupFiles pin DATABASE_URL pre-import; PRAGMA asserted — 1112 tests had been mutating the LIVE database). RENDER STACK ×6 — HFX-14 real streaming (stream:true, verbatim deltas — the fake 4-char chunker deleted every \n in transport since M10; tool_calls stitched by index) · HFX-15 react-markdown + remark-gfm (NEW dep) — pipe tables/headings/bold render (raw ##/**/| never reach the screen) · HFX-16 NEW src/lib/agent/narration.ts — narration segments keyed per text id (pre-tool 'Let me check…' no longer wiped, in UI AND next-turn history) · HFX-17 auto-scroll targets the Radix Viewport (the old content-div scrollTop was a no-op) · HFX-18 res.ok checked + inline error chip w/ Retry (toasts invisible — Toaster unmounted until PRD P0-1) · HFX-19 ONE close affordance (panel's duplicate X gone, SheetPrimitive.Close stays). tests: hfx-batch0.test NEW 19 (behavioral via real services + source-contract pins) + 3 existing suites updated for honest numbers (register-services wages/budget-actual; report-services daily-pnl; doc-parity-m6c PO complete→received) → **1131 vitest** (1112+19) · tsc src/ 0 · eval --static PASS · context_check 574→**581/581** NO DRIFT (+SPEC-M36.md +valuation.ts +narration.ts +hfx-batch0.test.ts +route_smoke_batch0.sh +tests/setup/pin-test-db.ts +tests/setup/global-setup.ts; tools 230 unchanged — zero new tools, all fixes are read/write-path repairs) · NEW route_smoke_batch0 **15/15** (live SSE reconstruction 67 deltas / 8 newlines survived; party-ledger+bills-register+jobwork+payments+daily-pnl render; RTGS/NEFT live; no 'billed' ghost) · LIVE browser-verified (agent panel: real HTML table Code|Name + 0 literal pipes; narration persists across 10 tool cards; Viewport 0px from bottom; ONE Close; screenshots m36-agent-panel-table.png + m36-agent-panel-markdown.png, VLM-confirmed). Historical: 2026-08-30 (session: m35 — SPEC-M35 SHIPPED (holidays digest adoption — the M28 OUT promise; fourth six-task run, task 6 FINAL): digest.ts EXTENDED — DigestShutdownRow + DIGEST_SHUTDOWN_WINDOW_DAYS=14 (exported) + getUpcomingHolidays joins the existing Promise.all (the M28 read reused verbatim, zero extra round-trips) + sections.shutdowns {windowDays, rows} + the text block ('Upcoming shutdowns (14d)' w/ TODAY vs d-away + the planning line; SILENT when empty — the M28 discipline) · the digest PAGE renders the amber shutdowns card (CalendarClock, days chips, TODAY variant, /masters/govt-holiday deep-link) above the gate section, hidden when empty · NEW agent tool get_daily_digest (tools 229→230, read/reports; returns the briefing text + section counts + shutdown rows) — THE PHASE-4.5 PROMISE RESTORED (the tool never survived the rollback era; the digest was service + page + cron only) · SYSTEM_PROMPT reports line carries it · tests: digest-holidays.test NEW 10 (in-window days-until + text block; 30d excluded; quiet → no text block; block-after-gate order; TODAY case; tool ×2; source pins ×3) → **1112 vitest** (1102+10) · tsc src/ 0 · eval --static PASS 15/15 · context_check 572→**574/574** NO DRIFT (+digest-holidays.test.ts +SPEC-M35.md; tools 230) · NEW route_smoke_m35 **12/12** (card + title + near holiday + 30d-excluded + calendar link + window caption + text block + quiet-hides + zero residue) · LIVE browser-verified (login → digest: amber card w/ Deepavali '6d away' + calendar link + text block; FULL-PAGE screenshot m35-digest-holidays.png, VLM-confirmed; SSR lesson: strip <!-- --> comment nodes before grepping interpolated captions). Historical: 2026-08-30 (session: m34 — SPEC-M34 SHIPPED (terms master feeding invoice print — gap-audit A3 frmTerms: 'Terms & conditions master (print blocks)' → AppOption-backed, feeding print; fourth six-task run, task 5): NEW printTerms(family) in print/fetchers.ts (the getCompanyName neighbor) — reads AppOption `print.terms.<family>`, newline split, blanks dropped, NO cache (admin edits must reflect on the very next print) + DEFAULT_TERMS_FALLBACK constant (the M8 hardcoded line, kept as the fresh-install default) · invoice fetcher: owned terms lines REPLACE the fallback when present; absent/empty → fallback (a fresh install is never term-less) · /admin/options help text mentions print.terms.invoice + the frmTerms lineage (the surface already manages the rows — zero new UI; group 'print') · ZERO new tools — create/update/list_app_option doors already exist (G5 satisfied; the update door is plan-then-commit; the master-service treats empty values as 'field not provided' — clearing = deleting the row, documented) · tests: print-terms.test NEW 11 (helper ×5; invoice WITHOUT→fallback / WITH→owned lines replace; agent-door plan→commit→flip; source pins ×3) → **1102 vitest** (1091+11) · tsc src/ 0 · eval --static PASS 15/15 · context_check 570→**572/572** NO DRIFT (+print-terms.test.ts +SPEC-M34.md) · zero tools/menu/routes/schema change (229/132/165/78 stay) · NEW route_smoke_m34 **12/12** · LIVE browser-verified (login → invoice print: 3 owned terms lines render, fallback correctly absent; FULL-PAGE screenshot m34-invoice-terms.png, VLM-confirmed the numbered terms block; zombie-chrome lesson: kill all agent-browser processes before a LIVE script + --full screenshots for below-the-fold proof). Historical: 2026-08-30 (session: m33 — SPEC-M33 SHIPPED (barcode bundle flow — gap-audit P3 #2, the SPEC-M9 §9 parked item 'barcode scanning in cutting'; fourth six-task run, task 4; the bundle DATA existed since M3 with zero VISUAL artifact): NEW src/lib/erp/print/barcode.ts — the vendored Code128 encoder (the M27 QR precedent, zero prod deps): code sets B+C with python-barcode's EXACT switching semantics (digit runs ≥4 → C via value 99; lone buffered odd digit flushes through TO_B 100; digit-LEADING text starts in C; START_C+TO_B collapses to START_B AND START_C+99 — the '99' digit pair is indistinguishable from TO_C — collapses to START_C) + checksum (startValue + Σ valueᵢ·i) mod 103 + code128Symbols/code128Modules + code128Svg (one <path>, 10-module quiet zones, crispEdges, human-readable text, honest non-Code-B throw) · VERIFIED BYTE-IDENTICAL to tests/fixtures/code128-reference.json ×14 samples (python-barcode 0.16.1's own build() output; the fixture generator scripts/gen_code128_fixture.py EXTENDED +8 samples for the C-start paths — the jsQR discipline, 1D edition) · TWO GATE CATCHES in the first hour: (1) the hand-typed CODES table had 46 drift errors → the table is now GENERATED from the fixture (the 'NEVER hand-typed' pin); (2) the TO_C collapse rule missing → '99' failed 13/14 until ported · PRINT: bundle-labels docType (cut order → one label card per bundle, 2-col grid) + bundle-label (reprint by bundleNo/barcode/id — safeDecode for the %2F path form) + PrintDoc.labels/PrintLabelCard + PrintSheet label-card grid (data-testid=label-cards/label-barcode-N) + DocPrintLink +label prop + the cut-order view 'Print bundle labels' door · the print-doc-map no-orphan invariant honestly AMENDED (NON_CONFIG_DOORS: the label docTypes ride the cut-order view link + the get_bundle tool; registry pins 21→23) · agent tool get_bundle (228→229, read/cutting — the scan reflex chat-reachable; pins ×9 files; SYSTEM_PROMPT cutting line) · FIXTURE SURGERY: the parallel-session db commit a390a30 had degraded the LPP PO regression fixture (orders kept exact pcs/values but lost 696GJ link + USD flag → eval --static 13/15) — scripts/repair_lpp_fixture.ts (idempotent) restores it; 15/15 again · tests: print-barcode.test NEW 19 (parity ×2 + test-local DECODER round-trip + valueB identity + C-start/'99'-collapse pins + SVG ×4 + fetchers ×4 service-level + registry/tool pins + source pins ×4) → **1091 vitest** (1072+19) · tsc src/ 0 · eval --static PASS 15/15 · context_check 565→**570/570** NO DRIFT (+barcode.ts +print-barcode.test.ts +code128-reference.json +gen_code128_fixture.py +SPEC-M33.md; print lib 9; print families 23) · NEW route_smoke_m33 **17/17** · LIVE browser-verified (login → cut order view → Print bundle labels → 3 cards + 3 Code128 SVGs + barcode text; zero console errors; screenshot m33-bundle-labels.png, VLM-confirmed). Historical: 2026-08-30 (session: m32 — SPEC-M32 SHIPPED (voice TTS confirm loop — the M24 OUT promise: the legacy voice confirm was READ-BACK; fourth six-task run, task 3): voice.ts EXTENDED — getSpeechSynthesis probe + PURE planSpeechText (the speakable read-back: summary + Creates/Updates counts + ≤3 side effects, 320-char cap, singular/plural) + speak (cancel-first so reads never stack; en-IN + rate 0.95 — plans are English; voice-by-lang-prefix; graceful no-ctor) + stopSpeaking · agent-panel: the 🔇/🔊 talking-panel toggle (data-testid=voice-speak-toggle beside the lang chip, localStorage fo.voiceSpeak, DEFAULT OFF — never surprise audio) + voiceSpeakRef mirror (the SSE closure never goes stale) + a pending write plan SPEAKS ITS PLAN at the isPending moment (eyes-busy/hands-busy shop floor) + Approve→'Committed.' / Reject→'Rejected.' spoken acks + panel-close stopSpeaking (the orphaned-AUDIO twin of the M24 mic stop) · OUT documented: server TTS, ta-IN plan speech, voice-command approval ('yes'), read-back of agent TEXT answers (plans are THE confirm moment) · tests: voice.test +15 (planSpeechText ×5; speak/stopSpeaking ×7 vs mocked engine incl. cancel-first + voice-pick + no-ctor; panel source pins ×3; the M24 single-surface invariant holds) → **1072 vitest** (1057+15) · tsc src/ 0 · eval --static PASS · context_check 564→**565/565** NO DRIFT (+SPEC-M32.md) · zero tools/menu/routes/schema change (228/132/165/78 stay) · LIVE browser-verified via scripts/m32_live_check.sh (the sandbox kills background processes between commands — one script, one flow: login → panel Voice/EN/🔇 → toggle ON → 🔊 + fo.voiceSpeak=1 → OFF → 0; zero console errors; screenshot m32-voice-confirm.png). Historical: 2026-08-30 (session: m31 — SPEC-M31 SHIPPED (working-day planner arithmetic — the M28 OUT promise: WF_PlanFinishDateArrival's Sunday+holiday skipping; fourth six-task run, task 2): holidays.ts EXTENDED — getHolidaysBetween shared read (the two M28 helpers deduped onto it) + PURE workingDayBreakdown (inclusive [from,to] window; holiday-on-Sunday counts ONCE as holiday; sundayWorking option — Tirupur units vary) + PURE addWorkingDays (the Nth working day AT-OR-AFTER from; maxScan guard 400 → honest null) + db wrappers workingDaysUntil (the delivery-promise runway) and planFinishDate (leadDays → finish date) · Order Hub: the Delivery tile carries 'N working days (S Suns, H shutdowns)' (data-testid=working-days, future promises on live orders) + the M28 amber strip gains 'Only N of M days before delivery are working days' · agent tool get_working_days (tools 227→228; window mode = breakdown + shutdown names, leadDays mode = finish date, sundayWorking param, honest missing-args) — 'how many working days until the 15th?' is now chat-reachable · HYGIENE: the runway surfaced TRIPLE GovtHoliday rows (the seed's holiday block wasn't idempotent — naive create ran 3×); seed.ts now findFirst-guards + scripts/dedupe_holidays.ts swept 6 dupes; the pure breakdown was always Set-deduped, only the warning display tripled · tests: holidays.test +21 (pure math incl. the Saturday-works lesson — 5 first-draft expectations were wrong, the code was right; wrappers; tool ×4; source pins) → **1057 vitest** (1036+21) · tsc src/ 0 · eval --static PASS · context_check 563→**564/564** NO DRIFT (+SPEC-M31.md pin; tools claim 228) · LIVE browser-verified (SO-1001: '39 working days (7 Suns, 1 shutdown)' + 'Only 39 of 47 days'; screenshot m31-working-days.png). Historical: 2026-08-30 (session: m30 — SPEC-M30 SHIPPED (legacy-forms alias hygiene, gap-audit §8-1 + §8-2/3/4 header drifts; fourth six-task run, task 1): the §8-1 audit re-run found **35** broken legacyForms refs (the '29' grew by six) — NEW src/lib/erp/legacy-aliases.ts classifies every one: 18 LEGACY_FORM_ALIASES (12 renames FrmOrderReg→FrmOrderRegister etc + 6 SQL-objects/report-files ST_Ord_inHand→FrmTradingOrdersInHandReg, Sp_POBalnce→FrmPartyBalanceRegister, Vue_StkLedger→FrmStockLedger, RPtFabDcRet→FrmFabDel_Return, FrmTallyExport→FrmTally_GSTSetup, frmOptionsFlags→frmOptions) + 17 NON_FORM_LEGACY (report files/stored procs/views/phantom spellings/our own M15-audit+M20-attendance inventions — searchable, NEVER counted); canonicalLegacyForm single-hop + countableLegacyForms (canonicalize→drop non-forms→dedup→sort — THE parity-count source) + searchableLegacyForms (raw+canonical palette expansion) · parityStats counts through countableLegacyForms (denominator honest: 272 unique refs → 249 countable canonical forms; coverage stays 100% — all items live) · parity page renders the honest count + (+N dropped) hint (LIVE: Rate Confirmation '0 (+3)', Closing Stock '0 (+2)', Party Balance 2, Order Register 3) · CommandPalette joins searchableLegacyForms — typing the REAL name FrmOrderRegister now finds the screen whose array carries only the abbreviation (LIVE-verified; raw mnemonic still works) · §8-2/3 header drifts fixed: menu-registry ITEMS 131→132 (test already pinned 132), reports/index.ts 15/13→16/12 split (+ the test description) · tests: legacy-aliases.test NEW 20 — THE COMPLETENESS INVARIANT reads docs/form-taxonomy.json and fails on any future unclassified ref; alias targets verified; collision/single-hop/idempotence; countable/searchable semantics; 5 consumer source pins) → **1036 vitest** (1016+20) · tsc src/ 0 · eval --static PASS · context_check 560→**563/563** NO DRIFT (+legacy-aliases.ts +legacy-aliases.test.ts +SPEC-M30.md) · zero menu/route/tool/schema change (132/165/227/78 stay) · LIVE browser-verified (screenshot m30-parity-aliases.png). Historical: 2026-08-30 (session: m29 — SPEC-M29 SHIPPED (the jump bar's G residual — gap-audit §7-G: 'type 1042 or SO-1042 → jump to doc; frmPcsDel → the screen; master records'; M18 shipped the palette over menu items, this closes the other three input kinds; third six-task run, task 6 FINAL): NEW src/lib/erp/jump.ts — the 12-family doc-number resolver (order/purchase-order/grn [model gRN — the tsc-caught casing]/invoice/despatch/cut/jobwork/journal/payment/debit-note/program/sample → numberField + view-route builder; resolveJump: bare digit run searches every family CONTAINS, prefixed query targets its family; ordering exact-equals → startsWith → contains; capped at 8; hrefs carry REAL db ids) + GET /api/erp?resource=jump (session-guarded, 400 missing q — the master_search neighbor) · CommandPalette upgrades: debounced 200ms dual fetch (jump + master_search party) on q≥2; a Documents group (ranked above Actions) + a Parties group linking /masters/party?q=<code>; legacyForms strings join every menu item's search value ('frmPcsDel' finds the Pcs DC screen — the data was already there, zero new infra); the placeholder teaches the syntax · masters ?q=: the [entity] page reads searchParams.q → MasterTable initialSearch (the '/' focus behavior unchanged) · OUT (documented): more palette master slugs, fuzzy matching, the 29 broken legacyForms strings (gap-audit §8-1 alias-map hygiene), doc-LINE jumps · tests: jump.test NEW 9 (12-family route-pattern audit; prefixed exact + real-id href; bare-digits contains + cap + exact-first; short/unknown = []; the api source pin; palette fetch/groups/debounce pins; legacyForms pin; masters ?q= pins; view-base whitelist) → **1016 vitest** (1007+9) · tsc src/ 0 · eval --static PASS · context_check 557→**560/560** NO DRIFT (+jump.ts +jump.test.ts +SPEC-M29.md pins) · NEW route_smoke_m29 **13/13** (401/400 guards; prefixed resolves family + real-id href + exact docNo; bare digits contains; unknown prefix = empty — parties are NOT docs; palette ships in the shell; masters ?q= prefills; zero residue) · LIVE browser-verified END-TO-END: ⌘K → type 'LIVE29-SO-…' → the Documents option appears → Enter lands /orders/<real-id> with the Order Hub showing the doc; ⌘K → 'frmPcsDel' → 'Pcs DC (Despatch)' surfaces; ZERO console errors, screenshot download/m29-jump-bar.png · docs: SPEC-M29 frozen+§5, this line, milestone row M29, next-actions #34, worklog Task 39) — (prior: m28 — holiday surfacing) — THIRD SIX-TASK RUN COMPLETE (M24 voice + M25 line-keypad + M26 IRN cancel + M27 print QR + M28 holidays + M29 jump bar)
Last verified: 2026-08-30 (session: m28 — SPEC-M28 SHIPPED (holiday calendar surfacing — gap-audit §7-H: 'GovtHoliday master exists: surface it in delivery promises and the MIS'; the master had existed since M2 with ZERO planning surfaces reading it; third six-task run, task 5): NEW src/lib/erp/holidays.ts — the planning read service: getUpcomingHolidays({from?, days=30}) → {date, name, daysUntil}[] (future-only, asc, midnight-normalized so a same-day holiday IS in-window) + holidaysBeforeDelivery(deliveryDate) → the promise-risk window [today, delivery] (a holiday AFTER delivery never threatens the promise; past/null delivery → empty; the rate-memory precedent — ERP-internal read, ZERO new schema/tools) · Order Hub /orders/[id]: amber 'Shutdown before delivery' strip (data-testid=holiday-warning) on open/in_progress orders when a holiday falls inside the delivery window — 'Pongal (2026-09-05, 6d away) — plan despatch & production around it'; silent when safe (no permanent empty box) · MIS Dashboard /reports/mis: 'Upcoming shutdowns' card (data-testid=holiday-strip, next 4 holidays w/ days-until chips + the /masters/govt-holiday calendar deep-link), hidden when empty · OUT (documented): working-day arithmetic in planners (WF_PlanFinishDateArrival's Sunday-skip — its own spec when a planner exists), program-date surfaces (no hard dates in schema), digest adoption, Sunday handling (units vary) · tests: holidays.test NEW 7 (window math + 45d-exclusion + empty case; delivery-window filter both directions + null/past; same-day midnight normalization; both page source pins incl. the open/in_progress guard) → **1007 vitest** (1000+7) · tsc src/ 0 · eval --static PASS · context_check 554→**557/557** NO DRIFT (+holidays.ts +holidays.test.ts +SPEC-M28.md pins) · NEW route_smoke_m28 **12/12** (holiday 6d out + risk order 10d out → warning strip + holiday named; safe order (3d delivery) stays silent; MIS strip + title + holiday + 61d-excluded; zero residue) · LIVE browser-verified: amber strip 'Shutdown before delivery: LIVE28 Pongal (2026-09-05, 6d away) — plan despatch & production around it' + MIS card w/ both holidays + calendar link, ZERO console errors, screenshot download/m28-holiday-surfacing.png · docs: SPEC-M28 frozen+§5, this line, milestone row M28, next-actions #33, worklog Task 38) — (prior: m27 — print QR)
Last verified: 2026-08-30 (session: m27 — SPEC-M27 SHIPPED (print QR — the QR lib decision RESOLVED: vendored single-file MIT encoder, zero production dependencies; third six-task run, task 4): NEW src/lib/erp/print/qr.ts — a faithful re-implementation of the qrcode-generator algorithm (Kazuhiko Arase, MIT — attribution in the header): byte mode · EC level M · versions 1–10 auto-selected (the 64-hex IRN lands on v5-M, 37×37) · RS error correction per the standard block table · alignment patterns v≥2 · format-info BCH(15,5) XOR 0x5412 + version-info BCH(18,6) v≥7 · 8-mask penalty auto-selection; exports qrMatrix(text) boolean[][] + qrSvg(text, sizePx) inline SVG (one <path> of dark modules, 4-module quiet zone in the viewBox, crispEdges, no canvas/no client JS — prints identically everywhere) · CROSS-VERIFICATION DISCIPLINE: jsqr@1.4.0 as a DEV-only dependency (production stays zero-dep) — tests/unit/print-qr.test.ts rasterizes the matrix to an RGBA buffer (8px/module) and DECODES it back with jsQR; THE GATE CAUGHT A REAL BUG IN ITS FIRST HOUR: the BCH remainder loop condition was bitLength(d) > deg(G) where the standard requires >= — an off-by-one leaving a 1-bit-wrong format-info remainder for exactly the boundary masks 1/3/4/6 (a haunting data-dependent pattern: len 10 failed while len 14 passed, same version; forced-mask probing isolated it in minutes); both formatBits and versionBits fixed; an 18-length × v1–v6 probe now decodes 100% · PRINT wiring: PrintDoc +qr/+qrLabel (additive); the invoice fetcher sets qr = qrSvg(inv.irn, 96) + label 'Scan to verify (mock IRN)' ONLY when a LIVE IRN exists (a cancelled IRN never prints — smoke-proven); PrintSheet renders the SVG beside the meta grid (data-testid=invoice-qr) · tests: print-qr.test NEW 8 (jsQR round-trips: the IRN @v5 + short @v1 + mid @v3; determinism; matrix size + separator/finder pins; SVG self-containment + too-long fail-safe; fetcher+sheet source pins) → **1000 vitest** (992+8) · tsc src/ 0 · eval --static PASS · context_check 551→**554/554** NO DRIFT (+qr.ts +print-qr.test.ts +SPEC-M27.md pins; print lib 7→8) · NEW route_smoke_m27 **14/14** (stamped print: QR block + svg + label + v5 viewBox 45×45 + meta rows; unstamped: no QR no IRN rows; cancel → QR leaves the print; cleanup) · LIVE browser-verified: the QR renders on a stamped invoice's print (svg count 1), zero console errors, screenshot download/m27-invoice-qr.png · docs: SPEC-M27 frozen+§5 record (corrected honestly: ONE bug caught, not two), this line, milestone row M27, next-actions #32, worklog Task 37) — (prior: m26 — IRN cancellation)
