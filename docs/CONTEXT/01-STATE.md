...
35. **QoL1 SURVEY DONE + P0 FIX SHIPPED** (2026-08-31, SPEC-QoL1): the agent/chat-stack QoL survey — 26 verified findings (P0×1 fixed, P1×4 correctness, P2×9 UX, P3×11 perf/hygiene) frozen as the M30–M32 fix plan. **P0-1 fixed same session**: cbd7c91 had accidentally deleted src/app/api/upload/route.ts (suite red, paperclip 404, guarded-routes drift 8→7) → restored verbatim from cbd7c91^ → **1016 vitest / context_check 560/560 NO DRIFT / eval --static PASS / tsc src 0** (report artifact regenerated by the static run). Key P1: the approval door executes RAW args while the proposal door executes zod-coerced args (parse-with-coercion.ts exists, is even duplicated inline in route.ts, but is wired into NEITHER door as designed) + approve has no plan-correlation (TOCTOU; plan.approvalId designed, never used) + malformed tool-call JSON kills the whole SSE turn + pre-tool narration is overwritten in the panel. New probe scripts/qol_prompt_sync.mjs (caught the prompt's ghost tool `accept_supplier_bill` + PROMPT_VERSION not bumped since M10 despite the M19 semantic change). **Next**: M30 chat-loop correctness (SPEC-QoL1 §4 D-1..D-4) → M31 chat UX (D-5..D-13) → M32 hygiene (D-14..D-17, incl. the PROMPT_VERSION bump + full-eval protocol run). **Sync note**: commit cbd7c91 + this work are LOCAL-ONLY — push blocked on the GitHub PAT (scrubbed everywhere by design; re-provide to push; method: docs/CONTEXT/03-PITFALLS.md line 55).
36. **M30 DONE — chat-loop correctness (SPEC-QoL1 P1 batch, SPEC-M30)** (2026-08-31): the four P1 fixes that change what actually *commits* through the chat door. **D-1** both doors execute IDENTICAL inputs — `normalizeArgs` + `parseWithCoercion` single-sourced in parse-with-coercion.ts (the route's 60-line inline duplicate deleted); loop events + audit rows carry the PARSED args. **D-2** malformed tool-args JSON no longer kills the SSE turn — the failure goes back to the model as an error tool-result (self-correct, the unknown-tool pattern). **D-3** approval correlation — the loop stamps a crypto-random `approvalId` into `plan.approvalId` + the AgentTurn row (additive column + index, models stay 78); the approve door requires it (unknown → 404, double-approve → 409 already_approved, plan-changed vs the persisted plan → 409 plan_changed with nothing committed — the TOCTOU guard) and the approved-marking updateMany is SCOPED (pre-M30 flipped every pending turn of the user). **D-4** transcript segments accumulate — SSE parsing + reduction moved to the pure module src/lib/agent/turn-events.ts (TranscriptReducer); pre-tool narration survives later text. **E** (found during extraction) a completion with content AND tool_calls pushes ONE assistant message (the old loop pushed two — duplicated narration in the model's own context). The loop itself moved to src/lib/agent/loop.ts (runAgentTurn, injectable LLM client + persist hook — the route is thin stream plumbing now; the M12 disconnect guard + M7B guard stay in the route). m7b/m15 smokes re-fitted (fixture inserts the proposal turn row, curl sends the token); NEW route_smoke_m30 18/18. **Gates**: 1052 vitest (1016→1052, +36: agent-loop 9 + approval-correlation 9 + turn-events 18) · tsc src/ 0 · eval --static PASS (prompt untouched — no full eval required by design, PROMPT_VERSION stays m10-2026-08-28; the bump is M32's own task) · context_check 560→579 NO DRIFT (MAX_STEPS/m10-import/m10-stamp pins re-pointed at loop.ts; 12 NEW m30 pins + 8 critical files) · route_smoke_m7b 26/26 + m15 13/13 re-run green. **LIVE browser-verified END-TO-END**: real GLM → "create supplier" → narration survives tool call ("I'll create…\n\nI've created…") → pending card → Approve → PRT-0006 committed + audit agent:create + turn approved=true approvedBy=admin + zero console errors (download/m30-live-approval.png). **PITFALLS candidates**: the dev server caches the OLD Prisma client in memory — after a `prisma db push`+generate, RESTART the dev server or routes 500 with "Unknown argument approvalId" (the unit tests see the new client, the long-running server doesn't); fixture `findFirst` without `orderBy: {createdAt: 'desc'}` tampered the WRONG row (the happy-path leftover) in route_smoke_m30. **Next**: M31 chat UX (SPEC-QoL1 D-5..D-13: true SSE streaming, react-markdown, transcript persistence, server-side abort, step-budget notice, history trim, committed-outcome card, data view + copy, smart autoscroll). **Sync note**: push still blocked on the PAT (3 local commits now: cbd7c91, cb79146, + the M30 commit).
