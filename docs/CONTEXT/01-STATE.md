...
Last verified: 2026-08-29 (session: p0 — KEYBOARD REFLEX PASS, the approved first slice of GAP-ANALYSIS-FIBERPRO §9 (user: "start with your choice"): ① doc-screen Enter NEVER implicit-submits — form-level keydown swallows Enter on INPUTs; last-row Enter spawns the successor row (data-cell coordinates + pendingFocus effect) and mid-grid Enter walks same-column-next-row; DocPicker's own Enter/Escape marked handled (stopPropagation) · ② F-keys on every DocScreen: F2 save (edit→plan, review→commit) / F4 next-empty-field walk (inputs + picker triggers via data-filled) / F9 open print route / Esc review→edit · ③ date header fields default to TODAY (localTodayISO en-CA local-time — not UTC-yesterday before 05:30 IST; initialHeader shared by useState + resetForAnother; prefill still wins) · ④ post-commit card gains Print CTA + F9 over the VERIFIED SLUG_PRINT_DOC map (20 slug→PRINT_DOCS pairs taken from live DocPrintLink usage; 'order' deliberately absent = the M13 order-sheet gap, pinned honest) · ⑤ keyboard-navigable tables: register-rows.tsx NEW client tbody (whole-row click, ↑↓/Home/End roving tabindex + emerald ring, Enter opens href — extracted from server RegisterScreen which delegates via <RegisterRows/>) + master-table useRowCursor (↑↓/Enter opens the edit sheet) · ⑥ 'Despatch & Logistics' group label + 7 dispatch breadcrumbs (terminology fidelity §7-T) · ⑦ agent-door tool-name chips removed from doc/register/report screens (agentTools data stays on configs for tests + Ask-agent prompts) · ⑧ global `/` in app-shell (document keydown; guards: no modifiers, not already in an editable; targets input[data-slash] — master search + register filter bar's first TEXT filter via slashIdx with date fallback; fallback: first visible search-flavoured input) — tests: keyboard-reflex.test.ts 20 NEW (print-map resolves vs PRINT_DOCS, order-absent pin, localTodayISO local-vs-UTC, initialHeader date/prefill, Despatch label + 7 breadcrumbs, source pins for Enter-swallow/F-keys/rows/slash per the auth.test readFileSync precedent — vitest node env can't mount components) → 744 vitest · context_check 410→418/418 NO DRIFT (views 29→30 +register-rows, +8 p0 checks) · route_smoke_m9 38/38 + m11 32/32 + m7b 25/25 · eval_routing --static PASS · tsc src/ 0 · next build EXIT 0 · browser-verified live via agent-browser: PO screen date=today, qty cell 100 + Enter → row spawned (4→8 data-cells) + focus on new row 1-0 + NO review card (the #1 reflex conflict is dead), F4 → first empty field, orders register ArrowDown×2 → ring on row 3 + Enter → Order Hub of exactly that order, master grid Enter → Edit sheet opened + Esc closed, `/` → rf-q on orders register + buyer search on masters, 'Despatch & Logistics' in sidebar, ZERO console errors, screenshot download/p0-orders-register.png · mid-verification bug found+fixed: fi===0 slash targeting missed configs whose first filter is a select (orders register) → slashIdx first-text-filter heuristic · also restored src/app/api/upload/route.ts (working tree had a spurious uncommitted deletion) and RECONSTRUCTED docs/GAP-ANALYSIS-FIBERPRO.md (the prior session's audit was lost in the sandbox reset before commit — same failure class as m11-convergence; provenance note inside) — (prior: m11 — the Feature-flags admin screen)
...
| P0 — Keyboard reflex pass | GAP-ANALYSIS-FIBERPRO §9 first slice (approved "start with your choice"): ① Enter row-spawn + implicit-submit swallow · ② F2/F4/F9/Esc on every DocScreen · ③ date-default-today (IST-safe) · ④ post-commit Print CTA over the verified SLUG_PRINT_DOC map (20 pairs) · ⑤ keyboard rows (register-rows.tsx NEW + master-table useRowCursor) · ⑥ Despatch spelling · ⑦ tool chips off · ⑧ global `/` | **COMPLETE** (`p0`): keyboard-reflex.test 20 → 744 vitest; context_check 418/418 (views 30, +8 p0 checks); smokes m9 38 + m11 32 + m7b 25; static eval PASS; tsc src/ 0; build EXIT 0; LIVE browser-verified (Enter spawns row + no submit, F4, arrows+Enter on registers/masters, `/`, Despatch label, zero console errors) |
...
| ERP view/shell components | **30** (23 + M6-C lifecycle-form.tsx + M6-D approval-queue.tsx + M8-A print trio + M9 live-tracker + P0 register-rows.tsx keyboard rows) | `ls src/components/erp/*.tsx \| wc -l` |
