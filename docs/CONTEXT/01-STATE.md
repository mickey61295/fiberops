...
Last verified: 2026-09-01 (session: m40 — SPEC-M40 SHIPPED (Phase-6B Batch 4 money integrity, the 7 PAY FRs + PAY-08 deferred per §17-3; SEAMS 3+4 CLOSED: PaymentAllocation FIFO settlement (invoice status DERIVES from Σ active allocations — two receipts that together cover the bill finally settle it), direction-correct invoice/bill links (out-payments attach SupplierBills, cross tags rejected with guidance), the SupplierBill document SB-#### (doc-config engine + create_supplier_bill + the REAL bill-pass gate — draft→passed with stored 3-way-match verdicts + TDS), the tolerance engine WIRED (bill_bcheck/grn_bal/entrydatedev/tds_default_percent finally consumed), honest AP (open bills − allocations; received-not-billed a labeled memo), money-voucher cancel/reversal with CN- contra legs (payment/journal/debit-note/expense/budget + invoice guards: live IRN + active allocations), due-date aging 0-30/31-60/61-90/90+ + on-account column): loop-closure #3 (wages ₹1000→₹600→owed ₹400 on the operator statement) + #4 (invoice ₹1000→₹400+₹600→paid via allocations) GREEN; 1245 vitest (+17); tsc src 0; eval --static PASS (m40-2026-09-01); context_check 604/604 NO DRIFT; route_smoke_m40 15/15 LIVE (SB view verdicts + register + 404 + zero residue); tools 238, docTools 62, menu 134, routes 168, models 83, schemas 42, posting 38, doc-configs 41.
